# fast_assignment_moodle_nodejs
A Moodle based app plugin which connects Node JS for pre IELTS examination sample evaluation check. Before students can submit their assignment, for some attempts they can evaluate their essay or grammar check. This connects with the Node version of Virtual Tutor API.
...
## Snippet
Proper ES8 standard of JavaScript is used. async-await functions is used for calling the APIs.
...
if (teacherPermission === 1 && isAdmin === 0) {
  $("#apimessage").html(
    "No. of automated evaluations available: " + remainingautoeval_hits
  );
}

if (teacherPermission === 0 && isAdmin === 0) {
  $("#id_automatedevaluation").hide();
}
...
const getTemplate = (result, testlink, type) => {
  ...
  if (type === "statistics") {
    ...
    let firstPersonPronounCheckText = firstPersonPronounCheck
      ? "have"
      : "have not";
  ...
  if (type === "paragraph") {
    ...
    for (key in paragraphWiseScoring) {
      ...
      for (key2 in outerData) {
        ...
        // template-append
        $(
          "id_fastassignfeedbackcomments_editoreditable, #id_fastassignfeedbackcomments_editor, .validation"
        ).append(`
                                                    <br />
  ...
  if (type === "vocabulary") {
    ...
    let vocabMatchedWords1 = vocabularylist1Check.matchedWords
      .toString()
      .trim();
    ...
    let vocabMatchedWords2 = vocabularylist2Check.matchedWords
      .toString()
      .trim();
...
$("#id_automatedevaluation").click(() => {
  ...
  if (dataToCheck == null) {
    ...
    $(".success").hide();
    $(".validation").append(
      "<h5 style='color:#ff0000'>Automated evaluation feedback </h5><img src='pix/suggestion.jpg' alt='suggestion' width='30'> This student have not submitted his assignment yet.</h5><br/><br/>"
    );
  } else {
    if (checkAvailableAutoevalAPI || isAdmin == 1) {
      ...
      automateEval(dataToCheck, testlink).then((response) => {
        ...
        if (status) {
          ...
          $(
            "id_fastassignfeedbackcomments_editoreditable, #id_fastassignfeedbackcomments_editor, .validation"
          ).append(templateScolarship);

          let templateLangAccuracy = getTemplate(
            result,
            testlink,
            "langaccuracy"
          );
        ...
        // when returns error from API
          ...
          $(
            "#id_fastassignfeedbackcomments_editoreditable, #id_fastassignfeedbackcomments_editor"
          ).append(`
                                            <h5 style='color:#ff0000'>Automated Evaluation: <img src='pix/suggestion.jpg' alt='suggestion' width='30'></h5>
                                            <p><b>Error from API: </b> <span style='color: #ff0000'>${error}</span></p>
                                        `);
      ...
      $(".success").hide();
      $(".validation").append(
        "<h5 style='color:#ff0000'>Automated evaluation feedback </h5><img src='pix/suggestion.jpg' alt='suggestion' width='30'> Your maximum no. of checks for automated evaluation API is finished.</h5><br/><br/>"
      );
    }
  }
});
