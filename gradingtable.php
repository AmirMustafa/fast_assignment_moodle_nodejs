 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_grading_table extends table_sql implements renderable {
    /** @var fastassignment $assignment */
     ...
     *
     * @param fastassignment $assignment The assignment class
     ...
     */
    public function __construct(fastassignment $assignment,
                                $perpage,
                                $filter,
                                $rowoffset,
                                $quickgrading,
        ...
        parent::__construct('mod_fastassignment_grading');
        $this->is_persistent(true);
        $this->assignment = $assignment;

        // Check permissions up front.
        $this->hasgrantextension = has_capability('mod/fastassignment:grantextension',
                                                  $this->assignment->get_context());
        $this->hasgrade = $this->assignment->can_grade();

        // Check if we have the elevated view capablities to see the blind details.
        $this->hasviewblind = has_capability('mod/fastassignment:viewblinddetails',
                $this->assignment->get_context());
        ...
        $this->output = $PAGE->get_renderer('mod_fastassignment');

        $urlparams = array('action' => 'grading', 'id' => $assignment->get_course_module()->id);
        $url = new moodle_url($CFG->wwwroot . '/mod/fastassignment/view.php', $urlparams);
        ...
        $params['newstatus'] = FASTASSIGN_SUBMISSION_STATUS_NEW;
        ...
        if ($hasoverrides) {
            ...
            JOIN (
              (SELECT 9999999 AS priority,
                      ...
                      a.cutoffdate
                 FROM {user} u
                 JOIN {fastassignment} a ON a.id = :assignmentid7
                 '.$userwhere2.'
              )
              UNION
            ...
            $fields .= 'a.cutoffdate ';
            $from .= 'JOIN {fastassignment} a ON a.id = :assignmentid5 ';
        }
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if ($filter == FASTASSIGN_FILTER_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
                                 s.status = :submitted) ';
                $params['submitted'] = FASTASSIGN_SUBMISSION_STATUS_SUBMITTED;

            } else if ($filter == FASTASSIGN_FILTER_NOT_SUBMITTED) {
                $where .= ' AND (s.timemodified IS NULL OR s.status <> :submitted) ';
                $params['submitted'] = FASTASSIGN_SUBMISSION_STATUS_SUBMITTED;
            } else if ($filter == FASTASSIGN_FILTER_REQUIRE_GRADING) {
                $where .= ' AND (s.timemodified IS NOT NULL AND
            ...
            } else if ($filter == FASTASSIGN_FILTER_GRANTED_EXTENSION) {
                $where .= ' AND uf.extensionduedate > 0 ';

            } else if (strpos($filter, FASTASSIGN_FILTER_SINGLE_USER) === 0) {
        ...
        if ($this->assignment->get_instance()->markingworkflow &&
            ...
            if (has_capability('mod/fastassignment:manageallocations', $this->assignment->get_context())) {
                // Check to see if marker filter is set.
                $markerfilter = (int)get_user_preferences('fastassignment_markerfilter', '');
                if (!empty($markerfilter)) {
                    if ($markerfilter == FASTASSIGN_MARKER_FILTER_NO_MARKER) {
        ...
        if ($this->assignment->get_instance()->markingworkflow) {
            ...
            if (!empty($workflowstates)) {
                ...
                if ($workflowfilter == FASTASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED) {
                    $where .= ' AND (uf.workflowstate = :workflowstate OR uf.workflowstate IS NULL OR '.
                        $DB->sql_isempty('fastassignment_user_flags', 'workflowstate', true, true).')';
        ...
        if ($this->hasviewblind || !$this->assignment->is_blind_marking()) {
            if (!$this->is_downloading()) {
                ...
                $columns[] = 'recordid';
                $headers[] = get_string('recordid', 'fastassignment');
            }
        ...
        // Submission status.
        $columns[] = 'status';
        $headers[] = get_string('status', 'fastassignment');

        if ($hasoverrides || $inrelativedatesmode) {
            // Allowsubmissionsfromdate.
            $columns[] = 'allowsubmissionsfromdate';
            $headers[] = get_string('allowsubmissionsfromdate', 'fastassignment');

            // Duedate.
            $columns[] = 'duedate';
            $headers[] = get_string('duedate', 'fastassignment');

            // Cutoffdate.
            $columns[] = 'cutoffdate';
            $headers[] = get_string('cutoffdate', 'fastassignment');
        }

        // Team submission columns.
        if ($assignment->get_instance()->teamsubmission) {
            $columns[] = 'team';
            $headers[] = get_string('submissionteam', 'fastassignment');
        }
        // Allocated marker.
        if ($this->assignment->get_instance()->markingworkflow &&
            $this->assignment->get_instance()->markingallocation &&
            has_capability('mod/fastassignment:manageallocations', $this->assignment->get_context())) {
            // Add a column for the allocated marker.
            $columns[] = 'allocatedmarker';
            $headers[] = get_string('marker', 'fastassignment');
        }
        // Grade.
        $columns[] = 'grade';
        ...
        if ($this->is_downloading()) {
            ...
            if ($gradetype > 0) {
                $columns[] = 'grademax';
                $headers[] = get_string('maxgrade', 'fastassignment');
            } else if ($gradetype < 0) {
                // This is a custom scale.
                $columns[] = 'scale';
                $headers[] = get_string('scale', 'fastassignment');
            }

            if ($this->assignment->get_instance()->markingworkflow) {
                // Add a column for the marking workflow state.
                $columns[] = 'workflowstate';
                $headers[] = get_string('markingworkflowstate', 'fastassignment');
            }
            // Add a column to show if this grade can be changed.
            $columns[] = 'gradecanbechanged';
            $headers[] = get_string('gradecanbechanged', 'fastassignment');
        }
        if (!$this->is_downloading() && $this->hasgrade) {
        ...
        if ($assignment->is_any_submission_plugin_enabled()) {
            ...
            $headers[] = get_string('lastmodifiedsubmission', 'fastassignment');
        ...
        // Time marked.
        $columns[] = 'timemarked';
        $headers[] = get_string('lastmodifiedgrade', 'fastassignment');
    ...
    public function col_recordid(stdClass $row) {
        ...
        return get_string('hiddenuser', 'fastassignment') . $row->recordid;
    }
    ...
    public function col_workflowstatus(stdClass $row) {
        ...
        if ($this->quickgrading && !$gradingdisabled) {
            ...
            $o .= $this->output->container(get_string('markingworkflowstate' . $workflowstate, 'fastassignment'), $workflowstate);
        }
        return $o;
    }
    ...
    public function col_workflowstate($row) {
        ...
        if (empty($state)) {
            $state = FASTASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED;
        }

        return get_string('markingworkflowstate' . $state, 'fastassignment');
    }
    ...
    public function col_allocatedmarker(stdClass $row) {
        ...
        if ($markers === null) {
            ...
            $markers = get_enrolled_users($this->assignment->get_context(), 'mod/fastassignment:grade', 0, 'u.*', $sort);
            $markerlist[0] = get_string('choosemarker', 'fastassignment');
        ...
        if ($this->quickgrading && has_capability('mod/fastassignment:manageallocations', $this->assignment->get_context()) &&
            (empty($row->workflowstate) ||
             $row->workflowstate == FASTASSIGN_MARKING_WORKFLOW_STATE_INMARKING ||
             $row->workflowstate == FASTASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED)) {
    ...
    public function col_team(stdClass $row) {
        ...
        if ($group) {
            ...
            if (count($usergroups) > 1) {
                return get_string('multipleteamsgrader', 'fastassignment');
            } else {
                return get_string('noteamgrader', 'fastassignment');
            }
        }
        return get_string('defaultteam', 'fastassignment');
    }
    ...
    public function col_select(stdClass $row) {
        ...
        $selectcol .= get_string('selectuser', 'fastassignment', $this->assignment->fullname($row));
        $selectcol .= '</label>';
    ...
    public function col_timesubmitted(stdClass $row) {
        ...
        $submission = false;
        $this->get_group_and_submission($row->id, $group, $submission, -1);
        if ($submission && $submission->timemodified && $submission->status != FASTASSIGN_SUBMISSION_STATUS_NEW) {
            $o = userdate($submission->timemodified);
        } else if ($row->timesubmitted && $row->status != FASTASSIGN_SUBMISSION_STATUS_NEW) {
    ...
    public function col_status(stdClass $row) {
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            ...
            $o .= $this->output->container(get_string('submissionstatus_' . $displaystatus, 'fastassignment'),
                                           array('class' => 'submissionstatus' .$displaystatus));
            if ($due && $timesubmitted > $due && $status != FASTASSIGN_SUBMISSION_STATUS_NEW) {
            ...
            if ($row->locked) {
                $lockedstr = get_string('submissionslockedshort', 'fastassignment');
            ...
            if (!$instance->markingworkflow) {
                if ($row->grade !== null && $row->grade >= 0) {
                    if ($row->timemarked < $row->timesubmitted) {
                        $o .= $this->output->container(get_string('gradedfollowupsubmit', 'fastassignment'), 'gradingreminder');
                    } else {
                        $o .= $this->output->container(get_string('graded', 'fastassignment'), 'submissiongraded');
                    }
                } else if (!$timesubmitted || $status == FASTASSIGN_SUBMISSION_STATUS_NEW) {
                    $now = time();
                    if ($due && ($now > $due)) {
                        $overduestr = get_string('overdue', 'fastassignment', format_time($now - $due));
        ...
        if ($row->extensionduedate) {
            ...
            $extensionstr = get_string('userextensiondate', 'fastassignment', $userdate);
    ...
    public function col_userid(stdClass $row) {
        ...
        if (!$row->grade) {
            ...
            $description = get_string('updategrade', 'fastassignment');
        }
        ...
        if ($this->assignment->is_any_submission_plugin_enabled()) {
            if (!$row->status ||
                    $row->status == FASTASSIGN_SUBMISSION_STATUS_DRAFT ||
                ...
                if (!$row->locked) {
                    ...
                    $description = get_string('preventsubmissionsshort', 'fastassignment');
                    ...
                    $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                       ...
                                       'page' => $this->currpage);
                    $url = new moodle_url('/mod/fastassignment/view.php', $urlparams);
                    $description = get_string('allowsubmissionsshort', 'fastassignment');
            ...
            if ($submissionsopen &&
                ...
                $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                   ...
                                   'page' => $this->currpage);
                $url = new moodle_url('/mod/fastassignment/view.php', $urlparams);
                $description = get_string('editsubmission', 'fastassignment');
            ...
            if ($USER->id != $row->id &&
                ...
                $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                   ...
                                   'page' => $this->currpage);
                $url = new moodle_url('/mod/fastassignment/view.php', $urlparams);
                $description = get_string('removesubmission', 'fastassignment');
        ...
        if (($this->assignment->get_instance()->duedate ||
             ...
             $urlparams = array('id' => $this->assignment->get_course_module()->id,
                                ...
                                'page' => $this->currpage);
             $url = new moodle_url('/mod/fastassignment/view.php', $urlparams);
             $description = get_string('grantextension', 'fastassignment');
        ...
        if ($row->status == FASTASSIGN_SUBMISSION_STATUS_SUBMITTED &&
                $this->assignment->get_instance()->submissiondrafts) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               'userid' => $row->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/fastassignment/view.php', $urlparams);
            $description = get_string('reverttodraftshort', 'fastassignment');
        ...
        if ($row->status == FASTASSIGN_SUBMISSION_STATUS_DRAFT &&
                $this->assignment->get_instance()->submissiondrafts &&
                $caneditsubmission &&
                $submissionsopen &&
                $row->id != $USER->id) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/fastassignment/view.php', $urlparams);
            $description = get_string('submitforgrading', 'fastassignment');
        ...
        $ismanual = $this->assignment->get_instance()->attemptreopenmethod == FASTASSIGN_ATTEMPT_REOPEN_METHOD_MANUAL;
        $hassubmission = !empty($row->status);
        $notreopened = $hassubmission && $row->status != FASTASSIGN_SUBMISSION_STATUS_REOPENED;
        $isunlimited = $this->assignment->get_instance()->maxattempts == FASTASSIGN_UNLIMITED_ATTEMPTS;
        ...
        if ($ismanual && $hassubmission && $notreopened && $hasattempts) {
            $urlparams = array('id' => $this->assignment->get_course_module()->id,
                               ...
                               'page' => $this->currpage);
            $url = new moodle_url('/mod/fastassignment/view.php', $urlparams);
            $description = get_string('addattempt', 'fastassignment');
     ...
     *
     * @param fastassignment_plugin $plugin Submission plugin or feedback plugin
     ...
     */
    private function format_plugin_summary_with_link(fastassignment_plugin $plugin,
                                                     stdClass $item,
                                                     $returnaction,
                                                     $returnparams) {
        $link = '';
        ...
        $separator = '';
        if ($showviewlink) {
            $viewstr = get_string('view' . substr($plugin->get_subtype(), strlen('fastassign')), 'fastassignment');
    ...
    public function other_cols($colname, $row) {
        ...
        if ($plugin->is_visible() && $plugin->is_enabled()) {
            if ($plugin->get_subtype() == 'fastassignsubmission') {
                if ($this->assignment->get_instance()->teamsubmission) {
                    $group = false;
                    $submission = false;
                    ...
                    if ($row->status == FASTASSIGN_SUBMISSION_STATUS_REOPENED) {
                        // For a newly reopened submission - we want to show the previous submission in the table.
