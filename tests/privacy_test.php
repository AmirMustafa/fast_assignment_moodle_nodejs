...
namespace mod_fastassignment\tests;
...
use \mod_fastassignment\privacy\provider;

/**
 * Unit tests for mod/fastassignment/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_fastassignment_privacy_testcase extends provider_testcase {
     ...
     */
    protected function create_submission($fastassignment, $user, $submissiontext, $attemptnumber = 0) {
        $submission = $fastassignment->get_user_submission($user->id, true, $attemptnumber);
    ...
    public function test_get_contexts_for_userid() {
        ...
        // fastassignment unique id for blind marking in assignment four for user 1.
        \fastassignment::get_uniqueid_for_user_static($assign4->get_instance()->id, $user1->id);
    ...
    /**
     * Test returning a list of user IDs related to a context (fastassignment).
     */
    public function test_get_users_in_context() {
        ...
        $comment = (object) [
            ...
            'component' => 'fastassignsubmission_comments',
        ...
        $userlist = new \core_privacy\local\request\userlist($context, 'fastassignment');
    ...
    public function test_export_user_data_student() {
        ...
        $submissiontext = 'My first submission';
        $submission = $this->create_submission($fastassignment, $user, $submissiontext);
        ...
        $submission = $this->create_submission($fastassignment, $user, $submissiontext2, 1);
        ...
        $data->attemptnumber = 1;
        $data->grade = $grade2;
        $data->fastassignfeedbackcomments_editor = ['text' => $teachercommenttext2, 'format' => FORMAT_MOODLE];
        ...
        $approvedlist = new approved_contextlist($user, 'mod_fastassignment', [$context->id, $coursecontext->id]);
        provider::export_user_data($approvedlist);

        // Check that we have general details about the assignment.
        $this->assertEquals('fastassignment 1', $writer->get_data()->name);
        // Check Submissions.
    ...
    public function test_export_user_data_teacher() {
        ...
        $submissiontext = 'My first submission';
        $submission = $this->create_submission($fastassignment, $user1, $submissiontext);
        ...
        // Give the submission a grade.
        $fastassignment->save_grade($user1->id, $data);

        // Create and grade some submissions from the students.
        $submissiontext2 = 'My first submission for user 2';
        $submission = $this->create_submission($fastassignment, $user2, $submissiontext2);
        ...
        // Give the submission a grade.
        $fastassignment->save_grade($user2->id, $data);

        // Create and grade some submissions from the students.
        $submissiontext3 = 'My second submission for user 2';
        $submission = $this->create_submission($fastassignment, $user2, $submissiontext3, 1);
        ...
        $data->attemptnumber = 1;
        $data->grade = $grade3;
        $data->fastassignfeedbackcomments_editor = ['text' => $teachercommenttext3, 'format' => FORMAT_MOODLE];
        ...
        // Set up some flags.
        $duedate = time();
        $flagdata = $fastassignment->get_user_flags($teacher->id, true);
        $flagdata->mailed = 1;
        $flagdata->extensionduedate = $duedate;
        $fastassignment->update_user_flags($flagdata);

        $writer = writer::with_context($context);
        $this->assertFalse($writer->has_any_data());

        // The student should have some text submitted.
        $approvedlist = new approved_contextlist($teacher, 'mod_fastassignment', [$context->id, $coursecontext->id]);
        provider::export_user_data($approvedlist);
    ...
    public function test_delete_data_for_all_users_in_context() {
        ...
        $overridedata->userid = $user2->id;
        $DB->insert_record('fastassignment_overrides', $overridedata);
        fastassignment_update_events($fastassignment);
        ...
        $records = $DB->get_records('fastassignment_submission');
        $this->assertEmpty($records);
        $records = $DB->get_records('fastassignment_grades');
        $this->assertEmpty($records);
        $records = $DB->get_records('fastassignsubmission_onlinetext');
        $this->assertEmpty($records);
        $records = $DB->get_records('fastassignfeedback_comments');
        $this->assertEmpty($records);

        // Check that overrides and the calendar events are deleted.
        $records = $DB->get_records('event');
        $this->assertEmpty($records);
        $records = $DB->get_records('fastassignment_overrides');
        $this->assertEmpty($records);
    }
    ...
    public function test_delete_data_for_user() {
        ...
        // Create and grade some submissions from the students.
        $submissiontext = 'My first submission';
        $submission1 = $this->create_submission($fastassignment, $user1, $submissiontext);
        ...
        // Create and grade some submissions from the students.
        $submissiontext2 = 'My first submission for user 2';
        $submission2 = $this->create_submission($fastassignment, $user2, $submissiontext2);
        ...
        // Create and grade some submissions from the students.
        $submissiontext3 = 'My second submission for user 2';
        $submission3 = $this->create_submission($fastassignment, $user2, $submissiontext3, 1);
        ...
        $approvedlist = new approved_contextlist($user2, 'mod_fastassignment', [$context->id, $coursecontext->id]);
        ...
        // Check calendar events as well as fastassignment overrides.
        $records = $DB->get_records('event');
        $this->assertCount(1, $records);
        $record = array_shift($records);
        // The remaining event should be for user 1.
        $this->assertEquals($user1->id, $record->userid);
        // Now for fastassignment_overrides
        $records = $DB->get_records('fastassignment_overrides');
        $this->assertCount(1, $records);
        $record = array_shift($records);
        // The remaining event should be for user 1.
    ...
    public function test_delete_data_for_users() {
        ...
        $data = $DB->get_records('fastassignment_submission');
        ...
        $data = $DB->get_records('fastassignment_grades');
        // Two entries in fastassignment_grades, one for each grade given.
        $this->assertCount(2, $data);

        $data = $DB->get_records('fastassignment_overrides');
        $this->assertCount(1, $data);

        $data = $DB->get_records('comments');
        $this->assertCount(1, $data);

        $userlist = new \core_privacy\local\request\approved_userlist($context, 'fastassignment', [$user1->id, $user2->id]);
        ...
        $userlist = new \core_privacy\local\request\approved_userlist($context, 'fastassignment', [$user3->id, $user5->id]);
        provider::delete_data_for_users($userlist);
