...
require_once($CFG->dirroot . '/mod/fastassignment/externallib.php');
require_once(__DIR__ . '/fixtures/testable_assign.php');

/**
 * External mod fastassignment functions unit tests
 *
 ...
 */
class mod_fastassignment_external_testcase extends externallib_advanced_testcase {
    ...
    public function test_get_grades() {
        ...
        $fastassignment = self::getDataGenerator()->create_module('fastassignment', $assigndata);
        ...
        $this->assignUserCapability('mod/fastassignment:viewgrades', $context->id, $roleid);
        ...
        $submission->assignment = $fastassignment->id;
        ...
        $DB->insert_record('fastassignment_submission', $submission);

        $grade = new stdClass();
        $grade->assignment = $fastassignment->id;
        ...
        $DB->insert_record('fastassignment_grades', $grade);
        ...
        $assignmentids[] = $fastassignment->id;
        $result = mod_fastassignment_external::get_grades($assignmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_grades_returns(), $result);

        // Check that the correct grade information for the student is returned.
        $this->assertEquals(1, count($result['assignments']));
        $assignment = $result['assignments'][0];
        $this->assertEquals($fastassignment->id, $assignment['assignmentid']);
    ...
    public function test_get_assignments() {
        ...
        $assign1 = self::getDataGenerator()->create_module('fastassignment', array(
            'course' => $course1->id,
        ...
        $filerecord = array('component' => 'mod_fastassignment', 'filearea' => 'intro', 'contextid' => $context->id, 'itemid' => 0,
        ...
        $context = context_module::instance($assign1->cmid);
        $this->assignUserCapability('mod/fastassignment:view', $context->id, $roleid);
        ...
        // Add a file as assignment attachment.
        $filerecord = array('component' => 'mod_fastassignment', 'filearea' => FASTASSIGN_INTROATTACHMENT_FILEAREA,
        ...
        $result = mod_fastassignment_external::get_assignments();

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_assignments_returns(), $result);
        ...
        $this->assertRegExp('@"' . $CFG->wwwroot . '/webservice/pluginfile.php/\d+/mod_fastassignment/intro/intro\.txt"@', $assignment['intro']);
        ...
        $DB->set_field('fastassignment', 'alwaysshowdescription', 0, array('id' => $assign1->id));
        $DB->set_field('fastassignment', 'allowsubmissionsfromdate', time() + DAYSECS, array('id' => $assign1->id));

        $result = mod_fastassignment_external::get_assignments(array($course1->id));
        ...
        $result = mod_fastassignment_external::get_assignments(array($course2->id));
        ...
        $result = mod_fastassignment_external::get_assignments(array($course1->id), array(), true);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_assignments_returns(), $result);
    ...
    public function test_get_assignments_with_submissionstatement() {
        ...
        $fastassignment = $this->getDataGenerator()->create_module('fastassignment', array(
            'course' => $course->id,
            'requiresubmissionstatement' => 1
        ));
        $assign2 = $this->getDataGenerator()->create_module('fastassignment', array('course' => $course->id));
        ...
        set_config('submissionstatement', $submissionstatement, 'fastassignment');
        ...
        // Check that the first fastassignment contains the submission statement.
        $assignmentret = $assignmentsret[0];
        $this->assertEquals($fastassignment->id, $assignmentret['id']);
        ...
        // Check that the second fastassignment does NOT contain the submission statement.
    ...
    public function test_get_submissions() {
        ...
        $assign1 = self::getDataGenerator()->create_module('fastassignment', $assigndata);
        ...
        $DB->insert_record('fastassignsubmission_onlinetext', $onlinetextsubmission);
        ...
        $result = mod_fastassignment_external::get_submissions($assignmentids);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_submissions_returns(), $result);
    ...
    public function test_get_user_flags() {
        ...
        $roleid = $this->assignUserCapability('moodle/course:viewparticipants', $context->id, 3);
        $context = context_module::instance($fastassignment->cmid);
        $this->assignUserCapability('mod/fastassignment:grade', $context->id, $roleid);
        ...
        $userflag->assignment = $fastassignment->id;
        ...
        $DB->insert_record('fastassignment_user_flags', $userflag);

        $assignmentids[] = $fastassignment->id;
        $result = mod_fastassignment_external::get_user_flags($assignmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_user_flags_returns(), $result);
    ...
    public function test_get_user_mappings() {
        ...
        $roleid = $this->assignUserCapability('moodle/course:viewparticipants', $context->id, 3);
        $context = context_module::instance($fastassignment->cmid);
        $this->assignUserCapability('mod/fastassignment:revealidentities', $context->id, $roleid);
        ...
        $mapping->assignment = $fastassignment->id;
        $mapping->userid = $student->id;

        $DB->insert_record('fastassignment_user_mapping', $mapping);

        $assignmentids[] = $fastassignment->id;
        $result = mod_fastassignment_external::get_user_mappings($assignmentids);

        // We need to execute the return values cleaning process to simulate the web service server.
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_user_mappings_returns(), $result);
    ...
    public function test_lock_submissions() {
        ...
        $params['fastassignsubmission_onlinetext_enabled'] = 1;
        ...
        $submission = $fastassignment->get_user_submission($student1->id, true);
        $data = new stdClass();
        ...
        $result = mod_fastassignment_external::lock_submissions($instance->id, $students);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::lock_submissions_returns(), $result);

        // Check for 0 warnings.
        $this->assertEquals(0, count($result));

        $this->setUser($student2);
        $submission = $fastassignment->get_user_submission($student2->id, true);
        $data = new stdClass();
        $data->onlinetext_editor = array('itemid'=>file_get_unused_draft_itemid(),
                                         'text'=>'Submission text',
                                         'format'=>FORMAT_MOODLE);
    ...
    public function test_unlock_submissions() {
        ...
        $result = mod_fastassignment_external::unlock_submissions($instance->id, $students);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::unlock_submissions_returns(), $result);
    ...
    public function test_submit_for_grading() {
        ...
        set_config('submissionreceipts', 0, 'fastassignment');
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_fastassignment');
        $params['course'] = $course->id;
        $params['fastassignsubmission_onlinetext_enabled'] = 1;
        ...
        $result = mod_fastassignment_external::submit_for_grading($instance->id, false);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::submit_for_grading_returns(), $result);

        // Should be 1 fail because the submission statement was not aceptted.
        $this->assertEquals(1, count($result));

        $result = mod_fastassignment_external::submit_for_grading($instance->id, true);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::submit_for_grading_returns(), $result);

        // Check for 0 warnings.
        $this->assertEquals(0, count($result));

        $submission = $fastassignment->get_user_submission($student1->id, false);

        $this->assertEquals(FASTASSIGN_SUBMISSION_STATUS_SUBMITTED, $submission->status);
    }
    ...
    public function test_save_user_extensions() {
        ...
        $this->setUser($teacher);
        $result = mod_fastassignment_external::save_user_extensions($instance->id, array($student1->id), array($now, $tomorrow));
        $result = external_api::clean_returnvalue(mod_fastassignment_external::save_user_extensions_returns(), $result);
        $this->assertEquals(1, count($result));

        $this->setUser($teacher);
        $result = mod_fastassignment_external::save_user_extensions($instance->id, array($student1->id), array($yesterday - 10));
        $result = external_api::clean_returnvalue(mod_fastassignment_external::save_user_extensions_returns(), $result);
        $this->assertEquals(1, count($result));

        $this->setUser($teacher);
        $result = mod_fastassignment_external::save_user_extensions($instance->id, array($student1->id), array($tomorrow));
        $result = external_api::clean_returnvalue(mod_fastassignment_external::save_user_extensions_returns(), $result);
        $this->assertEquals(0, count($result));
    ...
    public function test_reveal_identities() {
        ...
        $this->setUser($student1);
        $result = mod_fastassignment_external::reveal_identities($instance->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::reveal_identities_returns(), $result);
        $this->assertEquals(1, count($result));
    ...
    public function test_revert_submissions_to_draft() {
        ...
        $this->setUser($teacher);
        $students = array($student1->id, $student2->id);
        $result = mod_fastassignment_external::revert_submissions_to_draft($instance->id, array($student1->id));
        $result = external_api::clean_returnvalue(mod_fastassignment_external::revert_submissions_to_draft_returns(), $result);
    ...
    public function test_save_submission() {
        ...
        $params['course'] = $course->id;
        $params['fastassignsubmission_onlinetext_enabled'] = 1;
        $params['fastassignsubmission_file_enabled'] = 1;
        $params['fastassignsubmission_file_maxfiles'] = 5;
        $params['fastassignsubmission_file_maxsizebytes'] = 1024*1024;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('fastassignment', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_fastassignment_external::save_submission($instance->id, $submissionpluginparams);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::save_submission_returns(), $result);
        ...
        $this->assertEquals(get_string('duedatereached', 'fastassignment'), $result[0]['item']);
    }
    ...
    public function test_save_grade() {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_fastassignment');
        $params['course'] = $course->id;
        $params['fastassignfeedback_file_enabled'] = 1;
        $params['fastassignfeedback_comments_enabled'] = 1;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('fastassignment', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $feedbackeditorparams = array('text' => 'Yeeha!',
                                        'format' => 1);
        $feedbackpluginparams['fastassignfeedbackcomments_editor'] = $feedbackeditorparams;
        $result = mod_fastassignment_external::save_grade($instance->id,
                                                  $student1->id,
                                                  50.0,
                                                  -1,
                                                  true,
        ...
        $result = mod_fastassignment_external::get_grades(array($instance->id));
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_grades_returns(), $result);
    ...
    public function test_save_grades_with_advanced_grading() {
        ...
        $generator = $this->getDataGenerator()->get_plugin_generator('mod_fastassignment');
        $params['course'] = $course->id;
        $params['fastassignfeedback_file_enabled'] = 0;
        $params['fastassignfeedback_comments_enabled'] = 0;
        $instance = $generator->create_instance($params);
        $cm = get_coursemodule_from_instance('fastassignment', $instance->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_fastassignment_external::save_grades($instance->id, false, $grades);
        $this->assertNull($result);

        $student1grade = $DB->get_record('fastassignment_grades',
                                         array('userid' => $student1->id, 'assignment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
        $this->assertEquals((float)$student1grade->grade, '50.0');

        $student2grade = $DB->get_record('fastassignment_grades',
    ...
    public function test_save_grades_with_group_submission() {
        ...
        $result = mod_fastassignment_external::save_grades($instance->id, true, $grades1);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::save_grades_returns(), $result);
        ...
        $result = mod_fastassignment_external::save_grades($instance->id, true, $grades2);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::save_grades_returns(), $result);
        // There should be no warnings.
        $this->assertEquals(0, count($result));

        $student3grade = $DB->get_record('fastassignment_grades',
                                         array('userid' => $student3->id, 'assignment' => $instance->id),
                                         '*',
                                         MUST_EXIST);
        $this->assertEquals($student3grade->grade, '50.0');

        $student4grade = $DB->get_record('fastassignment_grades',
    ...
    public function test_copy_previous_attempt() {
        ...
        $params['course'] = $course->id;
        $params['fastassignsubmission_onlinetext_enabled'] = 1;
        $params['fastassignsubmission_file_enabled'] = 0;
        $params['fastassignfeedback_file_enabled'] = 0;
        ...
        $result = mod_fastassignment_external::copy_previous_attempt($instance->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::copy_previous_attempt_returns(), $result);
        // No warnings.
        $this->assertEquals(0, count($result));

        $this->setUser($teacher);
        $result = mod_fastassignment_external::get_submissions(array($instance->id));
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_submissions_returns(), $result);
    ...
    public function test_set_user_flags() {
        ...
        $createduserflags = mod_fastassignment_external::set_user_flags($fastassignment->id, $userflags);
        // We need to execute the return values cleaning process to simulate the web service server.
        $createduserflags = external_api::clean_returnvalue(mod_fastassignment_external::set_user_flags_returns(), $createduserflags);

        $this->assertEquals($student->id, $createduserflags[0]['userid']);
        $createduserflag = $DB->get_record('fastassignment_user_flags', array('id' => $createduserflags[0]['id']));

        // Confirm that all data was inserted correctly.
        $this->assertEquals($student->id,  $createduserflag->userid);
        $this->assertEquals($fastassignment->id, $createduserflag->assignment);
        ...
        $updateduserflags = mod_fastassignment_external::set_user_flags($fastassignment->id, $userflags);
        // We need to execute the return values cleaning process to simulate the web service server.
        $updateduserflags = external_api::clean_returnvalue(mod_fastassignment_external::set_user_flags_returns(), $updateduserflags);

        $this->assertEquals($student->id, $updateduserflags[0]['userid']);
        $updateduserflag = $DB->get_record('fastassignment_user_flags', array('id' => $updateduserflags[0]['id']));

        // Confirm that all data was updated correctly.
        $this->assertEquals($student->id,  $updateduserflag->userid);
        $this->assertEquals($fastassignment->id, $updateduserflag->assignment);
    ...
    public function test_view_grading_table_invalid_instance() {
        ...
        // Setup test data.
        $course = $this->getDataGenerator()->create_course();
        $fastassignment = $this->getDataGenerator()->create_module('fastassignment', array('course' => $course->id));
        $context = context_module::instance($fastassignment->cmid);
        $cm = get_coursemodule_from_instance('fastassignment', $fastassignment->id);

        // Test invalid instance id.
        mod_fastassignment_external::view_grading_table(0);
    }
    ...
    public function test_view_grading_table_not_enrolled() {
        ...
        mod_fastassignment_external::view_grading_table($fastassignment->id);
    }
    ...
    public function test_view_grading_table_correct() {
        ...
        $result = mod_fastassignment_external::view_grading_table($fastassignment->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::view_grading_table_returns(), $result);
        ...
        $moodleurl = new \moodle_url('/mod/fastassignment/view.php', array('id' => $cm->id));
    ...
    public function test_view_grading_table_without_capability() {
        ...
        $teacherrole = $DB->get_record('role', array('shortname' => 'teacher'));
        fastassignment_capability('mod/fastassignment:view', CAP_PROHIBIT, $teacherrole->id, $context->id);
    ...
    public function test_subplugins_availability() {
        ...
        $pluginmanager = new fastassignment_plugin_manager('fastassignsubmission');
        $pluginmanager->hide_plugin('file');
        $parameters = mod_fastassignment_external::save_submission_parameters();
        ...
        $pluginmanager = new fastassignment_plugin_manager('fastassignfeedback');
        $pluginmanager->hide_plugin('file');

        $parameters = mod_fastassignment_external::save_grade_parameters();
        ...
        $this->assertTrue(isset($parameters->keys['plugindata']->keys['fastassignfeedbackcomments_editor']));
        $this->assertEquals(VALUE_OPTIONAL, $parameters->keys['plugindata']->keys['fastassignfeedbackcomments_editor']->required);
    }
    ...
    public function test_view_submission_status() {
        ...
        // Test invalid instance id.
        try {
            mod_fastassignment_external::view_submission_status(0);
            $this->fail('Exception expected due to invalid mod_fastassignment instance id.');
        ...
        $this->setUser($user);
        try {
            mod_fastassignment_external::view_submission_status($fastassignment->id);
        ...
        $result = mod_fastassignment_external::view_submission_status($fastassignment->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::view_submission_status_returns(), $result);
        ...
        // Test user with no capabilities.
        // We need a explicit prohibit since this capability is only defined in authenticated user and guest roles.
        fastassignment_capability('mod/fastassignment:view', CAP_PROHIBIT, $studentrole->id, $context->id);
        accesslib_clear_all_caches_for_unit_testing();
        course_modinfo::clear_instance_cache();
    ...
    private function create_submission_for_testing_status($submitforgrading = false) {
        ...
        $params = array(
            'course' => $course->id,
            'fastassignsubmission_file_maxfiles' => 1,
            'fastassignsubmission_file_maxsizebytes' => 1024 * 1024,
            'fastassignsubmission_onlinetext_enabled' => 1,
            'fastassignsubmission_file_enabled' => 1,
            'submissiondrafts' => 1,
            'fastassignfeedback_file_enabled' => 1,
        ...
        if ($submitforgrading) {
            ...
            $fastassignment->submit_for_grading($data, $notices);
        }

        return array($fastassignment, $instance, $student1, $student2, $teacher, $group1, $group2);
    }
    ...
    public function test_get_submission_status_in_draft_status() {
        ...
        list($fastassignment, $instance, $student1, $student2, $teacher, $g1, $g2) = $this->create_submission_for_testing_status();
        $studentsubmission = $fastassignment->get_user_submission($student1->id, true);

        $result = mod_fastassignment_external::get_submission_status($fastassignment->get_instance()->id);
        // We expect debugging because of the $PAGE object, this won't happen in a normal WS request.
        $this->assertDebuggingCalled();

        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_submission_status_returns(), $result);
        ...
        $this->assertEquals($fastassignment->get_instance()->id, $result['lastattempt']['submission']['assignment']);
        ...
        list($expectedtext, $expectedformat) = external_format_text($onlinetext, FORMAT_HTML, $fastassignment->get_context()->id,
                'fastassignsubmission_onlinetext', FASTASSIGNSUBMISSION_ONLINETEXT_FILEAREA, $studentsubmission->id);
    ...
    public function test_get_submission_status_in_submission_status() {
        ...
        list($fastassignment, $instance, $student1, $student2, $teacher, $g1, $g2) = $this->create_submission_for_testing_status(true);
    ...
    public function test_get_submission_status_in_submission_status_for_teacher() {
        ...
        $this->setUser($teacher);
        // First one group.
        $result = mod_fastassignment_external::get_submission_status($fastassignment->get_instance()->id, 0, $g1->id);
        // We expect debugging because of the $PAGE object, this won't happen in a normal WS request.
        $this->assertDebuggingCalled();
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_submission_status_returns(), $result);
        ...
        // Second group.
        $result = mod_fastassignment_external::get_submission_status($fastassignment->get_instance()->id, 0, $g2->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_submission_status_returns(), $result);
        $this->assertCount(0, $result['warnings']);
        $this->assertEquals(1, $result['gradingsummary']['participantcount']);
        ...
        list($fastassignment, $instance, $student1, $student2, $teacher, $g1, $g2) = $this->create_submission_for_testing_status(false);
        $this->setUser($teacher);
        $result = mod_fastassignment_external::get_submission_status($fastassignment->get_instance()->id, 0, $g1->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_submission_status_returns(), $result);
        $this->assertCount(0, $result['warnings']);
    ...
    public function test_get_submission_status_in_reopened_status() {
        ...
        $fastassignment->testable_process_add_attempt($student1->id);
    ...
    public function test_get_submission_status_access_control() {
        ...
        // Access control test.
        mod_fastassignment_external::get_submission_status($fastassignment->get_instance()->id, $student1->id);
    ...
    public function test_get_submission_status_hidden_grader() {
        ...
        // Grade the fastassignment for the student1.
        $this->setUser($teacher);

        $data = new stdClass();
        $data->grade = '50.0';
        $data->fastassignfeedbackcomments_editor = ['text' => ''];
        $fastassignment->testable_apply_grade_to_user($data, $student1->id, 0);
        ...
        $result = mod_fastassignment_external::get_submission_status($fastassignment->get_instance()->id, $student1->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_submission_status_returns(), $result);
    ...
    public function test_get_submission_status_with_override() {
        ...
        $overridedata->assignid = $fastassignment->get_instance()->id;
        $overridedata->userid = $student1->id;
        $overridedata->allowsubmissionsfromdate = time() + YEARSECS;
        $DB->insert_record('fastassignment_overrides', $overridedata);
    ...
    public function test_get_participant_no_assignment() {
        $this->resetAfterTest(true);
        mod_fastassignment_external::get_participant('-1', '-1', false);
    }
    ...
    public function test_get_participant_no_view_capability() {
        ...
        $result = $this->create_fastassignment_with_student_and_teacher();
        $fastassignment = $result['fastassignment'];
        ...
        $this->setUser($student);
        fastassignment_capability('mod/fastassignment:view', CAP_PROHIBIT, $studentrole->id, $context->id, true);

        mod_fastassignment_external::get_participant($fastassignment->id, $student->id, false);
    }
    ...
    public function test_get_participant_no_grade_capability() {
        ...
        $this->setUser($teacher);
        fastassignment_capability('mod/fastassignment:viewgrades', CAP_PROHIBIT, $teacherrole->id, $context->id, true);
        fastassignment_capability('mod/fastassignment:grade', CAP_PROHIBIT, $teacherrole->id, $context->id, true);
        accesslib_clear_all_caches_for_unit_testing();
    ...
    public function test_get_participant_no_participant() {
        ...
        $result = $this->create_fastassignment_with_student_and_teacher(array('blindmarking' => true));
        ...
        $result = mod_fastassignment_external::get_participant($fastassignment->id, $student->id, false);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_participant_returns(), $result);
    }
    ...
    public function test_get_participant_blind_marking() {
        ...
        $result = mod_fastassignment_external::get_participant($fastassignment->id, $student->id, true);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_participant_returns(), $result);
    ...
    public function test_get_participant_no_user() {
        ...
        $result = $this->create_fastassignment_with_student_and_teacher();
        $assignmodule = $result['fastassignment'];
        $student = $result['student'];
        $teacher = $result['teacher'];
        $course = $result['course'];
        $context = context_course::instance($course->id);
        ...
        // Create an fastassignment instance to save a submission.
        set_config('submissionreceipts', 0, 'fastassignment');

        $cm = get_coursemodule_from_instance('fastassignment', $assignmodule->id);
        $context = context_module::instance($cm->id);
        ...
        $result = mod_fastassignment_external::get_participant($assignmodule->id, $student->id, false);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_participant_returns(), $result);
        $this->assertEquals($student->id, $result['id']);
    ...
    public function test_get_participant_group_submission() {
        ...
        $result = $this->create_fastassignment_with_student_and_teacher(array(
            'fastassignsubmission_onlinetext_enabled' => 1,
            'teamsubmission' => 1
        ));
        $assignmodule = $result['fastassignment'];
        ...
        $this->setUser($student);
        $submission = $fastassignment->get_group_submission($student->id, $group->id, true);
     ...
     * @param array $expectedproperties array of expected fastassignment properties.
     */
    public function test_get_participant_relative_dates(array $courseconfig, array $assignconfig, array $enrolconfig,
        ...
        $this->setUser($teacher);
        $result = mod_fastassignment_external::get_participant($fastassignment->get_instance()->id, $user->id, false);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::get_participant_returns(), $result);
    ...
    /**
     * Test for mod_fastassignment_external::list_participants().
     *
    ...
    public function test_list_participants_user_info_with_special_characters() {
        ...
        $data = $this->create_fastassignment_with_student_and_teacher();
        $assignment = $data['fastassignment'];
        ...
        $this->setUser($teacher);
        $participants = mod_fastassignment_external::list_participants($assignment->id, 0, '', 0, 0, false, true, true);
        $participants = external_api::clean_returnvalue(mod_fastassignment_external::list_participants_returns(), $participants);
        $this->assertCount(1, $participants);

        // Asser that we have a valid response data.
        $response = external_api::clean_returnvalue(mod_fastassignment_external::list_participants_returns(), $participants);
        ...
        $participants = mod_fastassignment_external::list_participants($assignment->id, 0, '', 0, 0, false, false, true);
        $participants = external_api::clean_returnvalue(mod_fastassignment_external::list_participants_returns(), $participants);
    ...
    /**
     * Test for the type of the user-related properties in mod_fastassignment_external::list_participants_returns().
     */
    public function test_list_participants_returns_user_property_types() {
        ...
        // Get returns description for mod_fastassignment_external::list_participants_returns().
        $listreturns = mod_fastassignment_external::list_participants_returns();
     ...
     */
    private function create_fastassignment_with_student_and_teacher($params = array()) {
        global $DB;
        ...
        // Create a course and assignment and users.
        $fastassignment = $this->getDataGenerator()->create_module('fastassignment', $params);
        ...
        fastassignment_capability('mod/fastassignment:view', CAP_ALLOW, $teacherrole->id, $context->id, true);
        fastassignment_capability('mod/fastassignment:viewgrades', CAP_ALLOW, $teacherrole->id, $context->id, true);
        fastassignment_capability('mod/fastassignment:grade', CAP_ALLOW, $teacherrole->id, $context->id, true);
        accesslib_clear_all_caches_for_unit_testing();

        return array(
            'course' => $course,
            'fastassignment' => $fastassignment,
            'student' => $student,
            'teacher' => $teacher
        );
    }
    ...
    public function test_view_assign() {
        ...
        $result = mod_fastassignment_external::view_assign($fastassignment->id);
        $result = external_api::clean_returnvalue(mod_fastassignment_external::view_fastassignment_returns(), $result);
