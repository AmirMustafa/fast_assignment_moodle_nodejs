 ...
 *
 * This class provides all the functionality for the new fastassignment module.
 *
...
define('FASTASSIGN_SUBMISSION_STATUS_NEW', 'new');
define('FASTASSIGN_SUBMISSION_STATUS_REOPENED', 'reopened');
define('FASTASSIGN_SUBMISSION_STATUS_DRAFT', 'draft');
define('FASTASSIGN_SUBMISSION_STATUS_SUBMITTED', 'submitted');

// Search filters for grading page.
define('FASTASSIGN_FILTER_NONE', 'none');
define('FASTASSIGN_FILTER_SUBMITTED', 'submitted');
define('FASTASSIGN_FILTER_NOT_SUBMITTED', 'notsubmitted');
define('FASTASSIGN_FILTER_SINGLE_USER', 'singleuser');
define('FASTASSIGN_FILTER_REQUIRE_GRADING', 'requiregrading');
define('FASTASSIGN_FILTER_GRANTED_EXTENSION', 'grantedextension');

// Marker filter for grading page.
define('FASTASSIGN_MARKER_FILTER_NO_MARKER', -1);

// Reopen attempt methods.
define('FASTASSIGN_ATTEMPT_REOPEN_METHOD_NONE', 'none');
define('FASTASSIGN_ATTEMPT_REOPEN_METHOD_MANUAL', 'manual');
define('FASTASSIGN_ATTEMPT_REOPEN_METHOD_UNTILPASS', 'untilpass');

// Special value means allow unlimited attempts.
define('FASTASSIGN_UNLIMITED_ATTEMPTS', -1);

// Special value means no grade has been set.
define('FASTASSIGN_GRADE_NOT_SET', -1);

// Grading states.
define('FASTASSIGN_GRADING_STATUS_GRADED', 'graded');
define('FASTASSIGN_GRADING_STATUS_NOT_GRADED', 'notgraded');

// Marking workflow states.
define('FASTASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED', 'notmarked');
define('FASTASSIGN_MARKING_WORKFLOW_STATE_INMARKING', 'inmarking');
define('FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORREVIEW', 'readyforreview');
define('FASTASSIGN_MARKING_WORKFLOW_STATE_INREVIEW', 'inreview');
define('FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORRELEASE', 'readyforrelease');
define('FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED', 'released');

/** FASTASSIGN_MAX_EVENT_LENGTH = 432000 ; 5 days maximum */
define("FASTASSIGN_MAX_EVENT_LENGTH", "432000");

// Name of file area for intro attachments.
define('FASTASSIGN_INTROATTACHMENT_FILEAREA', 'introattachment');

// Event types.
define('FASTASSIGN_EVENT_TYPE_DUE', 'due');
define('FASTASSIGN_EVENT_TYPE_GRADINGDUE', 'gradingdue');
define('FASTASSIGN_EVENT_TYPE_OPEN', 'open');
define('FASTASSIGN_EVENT_TYPE_CLOSE', 'close');

require_once($CFG->libdir . '/accesslib.php');
require_once($CFG->libdir . '/formslib.php');
require_once($CFG->dirroot . '/repository/lib.php');
require_once($CFG->dirroot . '/mod/fastassignment/mod_form.php');
require_once($CFG->libdir . '/gradelib.php');
require_once($CFG->dirroot . '/grade/grading/lib.php');
require_once($CFG->dirroot . '/mod/fastassignment/feedbackplugin.php');
require_once($CFG->dirroot . '/mod/fastassignment/submissionplugin.php');
require_once($CFG->dirroot . '/mod/fastassignment/renderable.php');
require_once($CFG->dirroot . '/mod/fastassignment/gradingtable.php');
require_once($CFG->libdir . '/portfolio/caller.php');

global $cm, $DB;


// echo '<script>var mydata = "testing developer";</script>';

use \mod_fastassignment\output\grading_app;

/**
 * Standard base class for mod_fastassignment (assignment types).
 *
 * @package   mod_fastassignment
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
...
class fastassignment {
    ...
    /** @var stdClass the assignment record that contains the global settings for this fastassignment instance */
    private $instance;
    ...
    /** @var grade_item the grade_item record for this fastassignment instance's primary grade item. */
    private $gradeitem;

    /** @var context the context of the course module for this fastassignment instance
     *               (or just the course if we are creating a new one)
     */
    private $context;

    /** @var stdClass the course this fastassignment instance belongs to */
    private $course;

    /** @var stdClass the admin config for all fastassignment instances  */
    private $adminconfig;

    /** @var fastassignment_renderer the custom renderer for this module */
    private $output;

    /** @var cm_info the course module for this fastassignment instance */
    ...
    /**
     * Constructor for the base fastassignment class.
     *
    ...
    public function __construct($coursemodulecontext, $coursemodule, $course) {
        ...
        $this->submissionplugins = $this->load_plugins('fastassignsubmission');
        $this->feedbackplugins = $this->load_plugins('fastassignfeedback');

        // Extra entropy is required for uniqid() to work on cygwin.
        $this->useridlistid = clean_param(uniqid('', true), PARAM_ALPHANUM);

        if (!isset($SESSION->mod_fastassignment_useridlist)) {
            $SESSION->mod_fastassignment_useridlist = [];
        }

        $coursemoduleData = $cm->instance;
        
        

        // $test_link_sql = $DB->get_record_sql("SELECT test_links	, test_name, category_name, intro FROM {fastassignment} WHERE id = $cm->instance");
        
        // $this->test_links_cookie = $test_link_sql->test_links;
        // $this->test_name_cookie = $test_link_sql->test_name;
        // $this->test_category_cookie = $test_link_sql->category_name;
        // $this->test_desc_cookie = $test_link_sql->intro;
    ...
    public function register_return_link($action, $params) {
        ...
        if ($cm) {
            $currenturl = new moodle_url('/mod/fastassignment/view.php', array('id' => $cm->id));
        } else {
            $currenturl = new moodle_url('/mod/fastassignment/index.php', array('id' => $this->get_course()->id));
        }
     ...
     *
     * @param string $subtype fastassignsubmission | fastassignfeedback
     * @param string $type
     * @return mixed fastassignment_plugin|null
     */
    public function get_plugin_by_type($subtype, $type) {
        $shortsubtype = substr($subtype, strlen('fastassign'));
     ...
     * @return mixed fastassignment_feedback_plugin|null
     */
    public function get_feedback_plugin_by_type($type) {
        return $this->get_plugin_by_type('fastassignfeedback', $type);
    }
     ...
     *
     * @param string $type - The type of plugin e.g comments
     * @return mixed fastassignment_submission_plugin|null
     */
    public function get_submission_plugin_by_type($type) {
        return $this->get_plugin_by_type('fastassignsubmission', $type);
    }
    ...
    public function load_plugins($subtype) {
        ...
        foreach ($names as $name => $path) {
            if (file_exists($path . '/locallib.php')) {
                ...
                $shortsubtype = substr($subtype, strlen('fastassign'));
                $pluginclass = 'fastassignment_' . $shortsubtype . '_' . $name;

                $plugin = new $pluginclass($this, $name);

                if ($plugin instanceof fastassignment_plugin) {
    ...
    public function view($action='', $args = array()) {
        ...
        if (!empty($action)) {
            $PAGE->set_pagetype('mod-fastassignment-' . $action);
        }
        // Now show the right view page.
        if ($action == 'redirect') {
            $nextpageurl = new moodle_url('/mod/fastassignment/view.php', $nextpageparams);
            $messages = '';
            ...
            $message = get_string('gradingchangessaved', 'fastassignment');
        ...
        } else if ($action == 'viewpluginfastassignfeedback') {
            $o .= $this->view_plugin_content('fastassignfeedback');
        } else if ($action == 'viewpluginfastassignsubmission') {
            $o .= $this->view_plugin_content('fastassignsubmission');
            ...
            $o .= $this->view_error_page(get_string('submitforgrading', 'fastassignment'), $notices);
    ...
    public function add_instance(stdClass $formdata, $callplugins) {
        ...
        // Fast Assignment data
        $update->api_key = $formdata->api_key;
        $update->api_post_url = $formdata->api_post_url;
        $update->test_links = $formdata->testlinks_hidden;
        $update->test_name = $formdata->test_name;
        $update->category_name = $formdata->category_name;
        $update->apiposturl_feedback = $formdata->apiposturl_feedback;
        $update->api_key_feedback = $formdata->api_key_feedback;
        $update->studentaccessibleautoeval = $formdata->studentaccessibleautoeval ? $formdata->studentaccessibleautoeval : 0;
        $update->teacheraccessibleautoeval = $formdata->teacheraccessibleautoeval ? $formdata->teacheraccessibleautoeval : 0;
        $update->automatedgrammarcheck = $formdata->automatedgrammarcheck ? $formdata->automatedgrammarcheck : 0;
        $update->maxnoofchecks = $formdata->maxnoofchecks;
        $update->maxnoofchecksstudent = $formdata->maxnoofchecksstudent;
        $update->maxnoofchecksteacher = $formdata->maxnoofchecksteacher;
        ...
        $update->attemptreopenmethod = FASTASSIGN_ATTEMPT_REOPEN_METHOD_NONE;
        ...
        $returnid = $DB->insert_record('fastassignment', $update);
        $this->instance = $DB->get_record('fastassignment', array('id'=>$returnid), '*', MUST_EXIST);
        ...
        if ($callplugins) {
            ...
            if (!empty($formdata->completionexpected)) {
                \core_completion\api::update_completion_date_event($formdata->coursemodule, 'fastassignment', $this->instance,
        ...
        $DB->update_record('fastassignment', $update);
    ...
    protected function delete_grades() {
        ...
        $result = grade_update('mod/fastassignment',
                               $this->get_course()->id,
                               'mod',
                               'fastassignment',
    ...
    public function delete_instance() {
        ...
        $DB->delete_records('fastassignment_submission', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('fastassignment_grades', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('fastassignment_plugin_config', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('fastassignment_user_flags', array('assignment' => $this->get_instance()->id));
        $DB->delete_records('fastassignment_user_mapping', array('assignment' => $this->get_instance()->id));
        ...
        $DB->delete_records('fastassignment', array('id'=>$this->get_instance()->id));

        return $result;
    }

    /**
     * Deletes a fastassignment override from the database and clears any corresponding calendar events
     *
    ...
    public function delete_override($overrideid) {
        ...
        if (empty($cm)) {
            ...
            $cm = get_coursemodule_from_instance('fastassignment', $instance->id, $instance->course);
        }

        $override = $DB->get_record('fastassignment_overrides', array('id' => $overrideid), '*', MUST_EXIST);

        // Delete the events.
        $conds = array('modulename' => 'fastassignment', 'instance' => $this->get_instance()->id);
        ...
        $DB->delete_records('fastassignment_overrides', array('id' => $overrideid));
        ...
        if (!empty($override->userid)) {
            ...
            $event = \mod_fastassignment\event\user_override_deleted::create($params);
        } else {
            $params['other']['groupid'] = $override->groupid;
            $event = \mod_fastassignment\event\group_override_deleted::create($params);
        }

        // Trigger the override deleted event.
        $event->add_record_snapshot('fastassignment_overrides', $override);
        $event->trigger();
    ...
    /**
     * Deletes all fastassignment overrides from the database and clears any corresponding calendar events
     */
    public function delete_all_overrides() {
        global $DB;

        $overrides = $DB->get_records('fastassignment_overrides', array('assignid' => $this->get_instance()->id), 'id');
    ...
    /**
     * Updates the fastassignment properties with override information for a user.
     *
     * Algorithm:  For each fastassignment setting, if there is a matching user-specific override,
     *   then use that otherwise, if there are group-specific overrides, return the most
     *   lenient combination of them.  If neither applies, leave the fastassignment setting unchanged.
     *
    ...
    public function update_effective_access($userid) {
        ...
        // Merge with fastassignment defaults.
    ...
    /**
     * Returns whether an fastassignment has any overrides.
     *
    ...
    public function has_overrides() {
        ...
        $override = $DB->record_exists('fastassignment_overrides', array('assignid' => $this->get_instance()->id));
     ...
     *   lowest sort order. If neither applies, leave the fastassignment setting unchanged.
     *
     * @param int $userid The userid.
    ...
    public function override_exists($userid) {
        ...
        $getuseroverride = function($userid) use ($DB) {
            $useroverride = $DB->get_record('fastassignment_overrides', ['assignid' => $this->get_instance()->id, 'userid' => $userid]);
        ...
        $getgroupoverride = function($userid) use ($DB) {
            ...
            $sql = "SELECT * FROM {fastassignment_overrides}
    ...
    public function is_override_calendar_event(\calendar_event $event) {
        ...
        if ($event->modulename != 'fastassignment') {
            return false;
        }
        ...
        if ($DB->get_record('fastassignment_overrides', $overrideparams)) {
            return true;
        } else {
            return false;
        }
    ...
    function get_valid_calendar_event_timestart_range(\calendar_event $event) {
        ...
        if ($event->eventtype == FASTASSIGN_EVENT_TYPE_DUE) {
            ...
            if ($submissionsfromdate) {
                $mindate = [
                    $submissionsfromdate,
                    get_string('duedatevalidation', 'fastassignment'),
                ];
            }
            ...
            if ($cutoffdate) {
                $maxdate = [
                    $cutoffdate,
                    get_string('cutoffdatevalidation', 'fastassignment'),
                ];
            }
            ...
            if ($gradingduedate) {
                ...
                if (!$cutoffdate || $gradingduedate < $cutoffdate) {
                    $maxdate = [
                        $gradingduedate,
                        get_string('gradingdueduedatevalidation', 'fastassignment'),
                    ];
                }
            }
            ...
            if ($duedate) {
                ...
                $mindate = [
                    $submissionsfromdate,
                    get_string('gradingduefromdatevalidation', 'fastassignment'),
                ];
            }
        }
    ...
    public function reset_userdata($data) {
        ...
        $componentstr = get_string('modulenameplural', 'fastassignment');
        $status = array();

        $fs = get_file_storage();
        if (!empty($data->reset_fastassignment_submissions)) {
            // Delete files associated with this assignment.
            foreach ($this->submissionplugins as $plugin) {
                $fileareas = array();
                ...
                if (!$plugin->delete_instance()) {
                    $status[] = array('component'=>$componentstr,
                                      'item'=>get_string('deleteallsubmissions', 'fastassignment'),
            ...
            $assignids = $DB->get_records('fastassignment', array('course' => $data->courseid), '', 'id');
            list($sql, $params) = $DB->get_in_or_equal(array_keys($assignids));

            $DB->delete_records_select('fastassignment_submission', "assignment $sql", $params);
            $DB->delete_records_select('fastassignment_user_flags', "assignment $sql", $params);
            ...
            if (!empty($data->reset_gradebook_grades)) {
                $DB->delete_records_select('fastassignment_grades', "assignment $sql", $params);
                // Remove all grades from gradebook.
                require_once($CFG->dirroot.'/mod/fastassignment/lib.php');
                fastassignment_reset_gradebook($data->courseid);
            }

            // Reset revealidentities for fastassignment if blindmarking is enabled.
            if ($this->get_instance()->blindmarking) {
                $DB->set_field('fastassignment', 'revealidentities', 0, array('id' => $this->get_instance()->id));
            }
        }

        // Remove user overrides.
        if (!empty($data->reset_fastassignment_user_overrides)) {
            $DB->delete_records_select('fastassignment_overrides',
                'assignid IN (SELECT id FROM {fastassignment} WHERE course = ?) AND userid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('useroverridesdeleted', 'fastassignment'),
                'error' => false);
        }
        // Remove group overrides.
        if (!empty($data->reset_fastassignment_group_overrides)) {
            $DB->delete_records_select('fastassignment_overrides',
                'assignid IN (SELECT id FROM {fastassignment} WHERE course = ?) AND groupid IS NOT NULL', array($data->courseid));
            $status[] = array(
                'component' => $componentstr,
                'item' => get_string('groupoverridesdeleted', 'fastassignment'),
                'error' => false);
        }

        // Updating dates - shift may be negative too.
        if ($data->timeshift) {
            $DB->execute("UPDATE {fastassignment_overrides}
            ...
            // See MDL-9367.
            shift_course_mod_dates('fastassignment',
     ...
     *
     * @param fastassignment_plugin $plugin The plugin to update
     * @param stdClass $formdata The form data
     * @return bool false if an error occurs
     */
    protected function update_plugin_instance(fastassignment_plugin $plugin, stdClass $formdata) {
    ...
    public function update_gradebook($reset, $coursemoduleid) {
        ...
        require_once($CFG->dirroot.'/mod/fastassignment/lib.php');
        $fastassignment = clone $this->get_instance();
        $fastassignment->cmidnumber = $coursemoduleid;

        // Set fastassignment gradebook feedback plugin status (enabled and visible).
        $fastassignment->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        ...
        return fastassignment_grade_item_update($fastassignment, $param);
    }
     ...
     * @return integer
     */
    public function get_fastassignment_perpage() {
        $perpage = (int) get_user_preferences('fastassignment_perpage', 10);
        $adminconfig = $this->get_admin_config();
        $maxperpage = -1;
    ...
    public function get_admin_config() {
        ...
        $this->adminconfig = get_config('fastassignment');
        return $this->adminconfig;
    }
    ...
    public function update_calendar($coursemoduleid) {
        ...
        if ($instance->duedate) {
            $event->name = get_string('calendardue', 'fastassignment', $instance->name);
            ...
            $select = "modulename = :modulename
                       ...
                       AND groupid = 0
                       AND courseid <> 0";
            $params = array('modulename' => 'fastassignment', 'instance' => $instance->id, 'eventtype' => $eventtype);
            ...
            $DB->delete_records('event', array('modulename' => 'fastassignment', 'instance' => $instance->id,
                'eventtype' => $eventtype));
        }

        $eventtype = FASTASSIGN_EVENT_TYPE_GRADINGDUE;
        if ($instance->gradingduedate) {
            $event->name = get_string('calendargradingdue', 'fastassignment', $instance->name);
            $event->eventtype = $eventtype;
            $event->timestart = $instance->gradingduedate;
            $event->timesort = $instance->gradingduedate;
            $event->id = $DB->get_field('event', 'id', array('modulename' => 'fastassignment',
    ...
    public function update_instance($formdata) {
        ...
        $result = $DB->update_record('fastassignment', $update);
        $this->instance = $DB->get_record('fastassignment', array('id'=>$update->id), '*', MUST_EXIST);
        ...
        \core_completion\api::update_completion_date_event($this->get_course_module()->id, 'fastassignment', $this->instance,
                $completionexpected);
    ...
    protected function save_intro_draft_files($formdata) {
        if (isset($formdata->introattachments)) {
            file_save_draft_area_files($formdata->introattachments, $this->get_context()->id,
                                       'mod_fastassignment', FASTASSIGN_INTROATTACHMENT_FILEAREA, 0);
        }
    }
     ...
     *
     * @param fastassignment_plugin $plugin The plugin to add the settings from
     ...
     * @return void
     */
    protected function add_plugin_settings(fastassignment_plugin $plugin, MoodleQuickForm $mform, & $pluginsenabled) {
        global $CFG;
    ...
    public function add_all_plugin_settings(MoodleQuickForm $mform) {
        $mform->addElement('header', 'submissiontypes', get_string('submissiontypes', 'fastassignment'));

        $submissionpluginsenabled = array();
        $group = $mform->addGroup(array(), 'submissionplugins', get_string('submissiontypes', 'fastassignment'), array(' '), false);
        ...
        $mform->addElement('header', 'feedbacktypes', get_string('feedbacktypes', 'fastassignment'));
        $feedbackpluginsenabled = array();
        $group = $mform->addGroup(array(), 'feedbackplugins', get_string('feedbacktypes', 'fastassignment'), array(' '), false);
        foreach ($this->feedbackplugins as $plugin) {
    ...
    protected function get_module_name() {
        ...
        self::$modulename = get_string('modulename', 'fastassignment');
        return self::$modulename;
    }
    ...
    protected function get_module_name_plural() {
        ...
        self::$modulenameplural = get_string('modulenameplural', 'fastassignment');
        return self::$modulenameplural;
    }
    ...
    public function get_default_instance() {
        ...
        if (!$this->instance && $this->get_course_module()) {
            ...
            $this->instance = $DB->get_record('fastassignment', $params, '*', MUST_EXIST);
    ...
    /**
     * Get the settings for the current instance of this assignment
     * @param int|null $userid the id of the user to load the fastassignment instance for.
     * @return stdClass The settings
     */
     ...
     *
     * @param stdClass $record the raw fastassignment record.
    ...
    /**
     * Get the primary grade item for this fastassignment instance.
     *
    ...
    protected function count_attachments() {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($this->get_context()->id, 'mod_fastassignment', FASTASSIGN_INTROATTACHMENT_FILEAREA,
                        0, 'id', false);
        ...
        if ($this->get_instance()->grade >= 0) {
            ...
            if ($editing && $this->get_instance()->grade > 0) {
                ...
                $o .= '<label class="accesshide" for="quickgrade_' . $userid . '">' .
                       get_string('usergrade', 'fastassignment') .
                       '</label>';
                $o .= '<input type="text"
    ...
    private function get_submission_info_for_participants($participants) {
        ...
        $from = ' FROM {user} u
                         LEFT JOIN {fastassignment_submission} s
                                ON u.id = s.userid
                               AND s.assignment = :assignmentid1
                               AND s.latest = 1
                         LEFT JOIN {fastassignment_grades} g
                                ON u.id = g.userid
                               AND g.assignment = :assignmentid2
                               AND g.attemptnumber = s.attemptnumber
                         LEFT JOIN {fastassignment_user_flags} uf
                                ON u.id = uf.userid
        ...
        if (!empty($this->get_instance()->blindmarking)) {
            $from .= 'LEFT JOIN {fastassignment_user_mapping} um
                             ON u.id = um.userid
        ...
        foreach ($records as $userid => $submissioninfo) {
            ...
            if (!empty($submissioninfo->stime) && $submissioninfo->status == FASTASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                $submitted = true;
            }
    ...
    private function get_grading_sort_sql() {
        $usersort = flexible_table::get_sort_for_table('mod_fastassignment_grading');
    ...
    public function list_participants($currentgroup, $idsonly, $tablesort = false) {
        ...
        if (!isset($this->participants[$key])) {
            list($esql, $params) = get_enrolled_sql($this->context, 'mod/fastassignment:submit', $currentgroup,
            ...
            if (!empty($instance->blindmarking)) {
                $additionaljoins .= " LEFT JOIN {fastassignment_user_mapping} um
                                  ON u.id = um.userid
            ...
            if ($instance->markingworkflow &&
                    ...
                    !has_capability('mod/fastassignment:manageallocations', $this->get_context()) &&
                    has_capability('mod/fastassignment:grade', $this->get_context())) {

                $additionaljoins .= ' LEFT JOIN {fastassignment_user_flags} uf
                                     ON u.id = uf.userid
                                     AND uf.assignment = :assignmentid3';
    ...
    public function get_participant($userid) {
        ...
        if (!is_enrolled($this->context, $participant, 'mod/fastassignment:submit', $this->show_only_active_users())) {
            return null;
        }
    ...
    public function count_submissions_need_grading($currentgroup = null) {
        ...
        list($esql, $params) = get_enrolled_sql($this->get_context(), 'mod/fastassignment:submit', $currentgroup, true);
        ...
        $sql = 'SELECT COUNT(s.userid)
                   FROM {fastassignment_submission} s
                   LEFT JOIN {fastassignment_grades} g ON
    ...
    public function count_submissions($includenew = false) {
        ...
        if (!$includenew) {
            ...
            $params['status'] = FASTASSIGN_SUBMISSION_STATUS_NEW;
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $currentgroup = groups_get_activity_group($this->get_course_module(), true);
            list($esql, $enrolparams) = get_enrolled_sql($this->get_context(), 'mod/fastassignment:submit', $currentgroup, true);
    ...
    protected function get_grading_userid_list($cached = false, $useridlistid = '') {
        ...
        if ($cached) {
            ...
            if (empty($SESSION->mod_fastassignment_useridlist[$useridlistkey])) {
                $SESSION->mod_fastassignment_useridlist[$useridlistkey] = $this->get_grading_userid_list(false, '');
            }
            return $SESSION->mod_fastassignment_useridlist[$useridlistkey];
        }
        $filter = get_user_preferences('fastassignment_filter', '');
        $table = new fastassignment_grading_table($this, 0, $filter, 0, false);
    ...
    public static function cron() {
        ...
        $timenow   = time();
        $lastruntime = $DB->get_field('task_scheduled', 'lastruntime', array('component' => 'mod_fastassignment'));
        ...
        $sql = "SELECT g.id as gradeid, a.course, a.name, a.blindmarking, a.revealidentities, a.hidegrader,
                 ...
                 FROM {fastassignment} a
                 JOIN {fastassignment_grades} g ON g.assignment = a.id
            LEFT JOIN {fastassignment_user_flags} uf ON uf.assignment = a.id AND uf.userid = g.userid
                 JOIN {course_modules} cm ON cm.course = a.course AND cm.instance = a.id
                 JOIN {modules} md ON md.id = cm.module AND md.name = 'fastassignment'
                 JOIN {grade_items} gri ON gri.iteminstance = a.id AND gri.courseid = a.course AND gri.itemmodule = md.name
            LEFT JOIN {fastassignment_user_mapping} um ON g.id = um.userid AND um.assignment = a.id
        ...
        $params = array(
            'yesterday' => $yesterday,
            'today' => $timenow,
            'wfreleased' => FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED,
        );
        ...
        if (!empty($submissions)) {
            ...
            foreach ($submissions as $submission) {
                ...
                if ($submission->hidegrader && !has_capability('mod/fastassignment:showhiddengrader', $contextmodule, $user)) {
                ...
                $eventtype = 'fastassignment_notification';
                $updatetime = $submission->lastmodified;
                $modulename = get_string('modulename', 'fastassignment');
                ...
                $flags = $DB->get_record('fastassignment_user_flags', array('userid'=>$user->id, 'assignment'=>$submission->assignment));
                if ($flags) {
                    $flags->mailed = 1;
                    $DB->update_record('fastassignment_user_flags', $flags);
                } else {
                    $flags = new stdClass();
                    $flags->userid = $user->id;
                    $flags->assignment = $submission->assignment;
                    $flags->mailed = 1;
                    $DB->insert_record('fastassignment_user_flags', $flags);
                }
        ...
        foreach ($newlyavailable as $record) {
            $cm = get_coursemodule_from_instance('fastassignment', $record->id, 0, false, MUST_EXIST);
            $context = context_module::instance($cm->id);
    ...
    public function update_user_flags($flags) {
        ...
        $result = $DB->update_record('fastassignment_user_flags', $flags);
        return $result;
    }
    ...
    public function update_grade($grade, $reopenattempt = false) {
        ...
        $DB->update_record('fastassignment_grades', $grade);
        ...
        if ($this->gradebook_item_update(null, $grade)) {
            \mod_fastassignment\event\submission_graded::create_from_grade($this, $grade)->trigger();
        }
    ...
    protected function view_grant_extension($mform) {
        global $CFG;
        require_once($CFG->dirroot . '/mod/fastassignment/extensionform.php');
        ...
        if (empty($mform)) {
            $mform = new mod_fastassignment_extension_form(null, $formparams);
        }
        $mform->set_data($data);
        $header = new fastassignment_header($this->get_instance(),
                                    $this->get_context(),
                                    $this->show_intro(),
                                    $this->get_course_module()->id,
                                    get_string('grantextension', 'fastassignment'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new fastassignment_form('extensionform', $mform));
        $o .= $this->view_footer();
        return $o;
    }
    ...
    public function get_submission_group_members_who_have_not_submitted($groupid, $onlyids) {
        ...
        foreach ($members as $id => $member) {
            ...
            if ($submission && $submission->status == FASTASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                unset($members[$id]);
            } else {
                if ($this->is_blind_marking()) {
                    $members[$id]->alias = get_string('hiddenuser', 'fastassignment') .
    ...
    public function get_group_submission($userid, $groupid, $create, $attemptnumber=-1) {
        ...
        $submission = null;
        $submissions = $DB->get_records('fastassignment_submission', $params, 'attemptnumber DESC', '*', 0, 1);
        if ($submissions) {
        ...
        if ($create) {
            ...
            if ($attemptnumber == -1) {
                ...
                $result = $DB->get_records('fastassignment_submission', $params, 'attemptnumber DESC', 'attemptnumber', 0, 1);
                if ($result) {
            ...
            if ($submission->latest) {
                ...
                $DB->set_field('fastassignment_submission', 'latest', 0, $params);
            }
            $submission->status = FASTASSIGN_SUBMISSION_STATUS_NEW;
            $sid = $DB->insert_record('fastassignment_submission', $submission);
            return $DB->get_record('fastassignment_submission', array('id' => $sid));
        }
        return false;
    }
    ...
    private function view_course_index() {
        ...
        if (!$cms = get_coursemodules_in_course('fastassignment', $course->id, 'm.duedate')) {
        ...
        $courseindexsummary = new fastassignment_course_index_summary($usesections, $strsectionname);

        $timenow = time();

        $currentsection = '';
        foreach ($modinfo->instances['fastassignment'] as $cm) {
            if (!$cm->uservisible) {
                continue;
            }
            ...
            if (has_capability('mod/fastassignment:grade', $context)) {
                $submitted = $assignment->count_submissions_with_status(FASTASSIGN_SUBMISSION_STATUS_SUBMITTED);

            } else if (has_capability('mod/fastassignment:submit', $context)) {
                ...
                if (!empty($usersubmission->status)) {
                    $submitted = get_string('submissionstatus_' . $usersubmission->status, 'fastassignment');
                } else {
                    $submitted = get_string('submissionstatus_', 'fastassignment');
                }
            }
            $gradinginfo = grade_get_grades($course->id, 'mod', 'fastassignment', $cm->instance, $USER->id);
            ...
            $courseindexsummary->add_fastassignment_info($cm->id, $cm->get_formatted_name(), $sectionname, $timedue, $submitted, $grade);
    ...
    protected function view_plugin_content($pluginsubtype) {
        ...
        $plugintype = required_param('plugin', PARAM_PLUGIN);
        $item = null;
        if ($pluginsubtype == 'fastassignsubmission') {
            ...
            $o .= $this->get_renderer()->render(new fastassignment_header($this->get_instance(),
                                                              $this->get_context(),
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new fastassignment_submission_plugin_submission($plugin,
                                                              $item,
                                                              fastassignment_submission_plugin_submission::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing a submission.
            \mod_fastassignment\event\submission_viewed::create_from_submission($this, $item)->trigger();
            ...
            $o .= $this->get_renderer()->render(new fastassignment_header($this->get_instance(),
                                                              ...
                                                              $this->get_course_module()->id,
                                                              $plugin->get_name()));
            $o .= $this->get_renderer()->render(new fastassignment_feedback_plugin_feedback($plugin,
                                                              $item,
                                                              fastassignment_feedback_plugin_feedback::FULL,
                                                              $this->get_course_module()->id,
                                                              $this->get_return_action(),
                                                              $this->get_return_params()));

            // Trigger event for viewing feedback.
            \mod_fastassignment\event\feedback_viewed::create_from_grade($this, $item)->trigger();
        }
     ...
     * @param fastassignment_plugin $plugin - The assignment plugin
     */
    public function download_rewrite_pluginfile_urls($text, $user, $plugin) {
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            if ($submissiongroup) {
                ...
                $groupname = get_string('defaultteam', 'fastassignment') . '-';
            }
        }

        if ($this->is_blind_marking()) {
            $prefix = $groupname . get_string('participant', 'fastassignment');
        ...
        if (!get_user_preferences('fastassignment_downloadasfolders', 1)) {
    ...
    public function render_editor_content($filearea, $submissionid, $plugintype, $editor, $component, $shortentext = false) {
        ...
        if ($CFG->enableportfolios && has_capability('mod/fastassignment:exportownsubmission', $this->context)) {
            ...
            $portfolioparams = array('cmid' => $this->get_course_module()->id,
                                     ...
                                     'editor' => $editor,
                                     'area'=>$filearea);
            $button->set_callback_options('fastassignment_portfolio_caller', $portfolioparams, 'mod_fastassignment');
            $fs = get_file_storage();
    ...
    protected function view_savegrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new fastassignment_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('savegradingresult', 'fastassignment')));
        $gradingresult = new fastassignment_gradingmessage(get_string('savegradingresult', 'fastassignment'),
                                                   $message,
                                                   $this->get_course_module()->id);
    ...
    protected function view_quickgrading_result($message) {
        ...
        $o .= $this->get_renderer()->render(new fastassignment_header($this->get_instance(),
                                                      ...
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      get_string('quickgradingresult', 'fastassignment')));
        $gradingerror = in_array($message, $this->get_error_messages());
        $lastpage = optional_param('lastpage', null, PARAM_INT);
        $gradingresult = new fastassignment_gradingmessage(get_string('quickgradingresult', 'fastassignment'),
                                                   $message,
                                                   $this->get_course_module()->id,
                                                   $gradingerror,
                                                   $lastpage);
    ...
    public function require_view_group_submission($groupid) {
        if (!$this->can_view_group_submission($groupid)) {
            throw new required_capability_exception($this->context, 'mod/fastassignment:viewgrades', 'nopermission', '');
        }
    }
    ...
    public function can_view_grades($groupid = null) {
        // Permissions check.
        if (!has_any_capability(array('mod/fastassignment:viewgrades', 'mod/fastassignment:grade'), $this->context)) {
            return false;
        }
    ...
    public function can_grade($user = null) {
        // Permissions check.
        if (!has_capability('mod/fastassignment:grade', $this->context, $user)) {
            return false;
        }
    ...
    protected function download_submissions($userids = false) {
        ...
        $students = get_enrolled_users($this->context, "mod/fastassignment:submit", null, 'u.*', null, null, null,
                        $this->show_only_active_users());
        ...
        foreach ($students as $student) {
            ...
            if ((groups_is_member($groupid, $userid) or !$groupmode or !$groupid)) {
                ...
                if ($this->is_blind_marking()) {
                    $prefix = str_replace('_', ' ', $groupname . get_string('participant', 'fastassignment'));
                ...
                if ($submission) {
                    $downloadasfolders = get_user_preferences('fastassignment_downloadasfolders', 1);
                    foreach ($this->submissionplugins as $plugin) {
                        if ($plugin->is_enabled() && $plugin->is_visible()) {
        ...
        if (count($filesforzipping) == 0) {
            $header = new fastassignment_header($this->get_instance(),
                                        ...
                                        '',
                                        $this->get_course_module()->id,
                                        get_string('downloadall', 'fastassignment'));
            $result .= $this->get_renderer()->render($header);
            $result .= $this->get_renderer()->notification(get_string('nosubmission', 'fastassignment'));
            $url = new moodle_url('/mod/fastassignment/view.php', array('id'=>$this->get_course_module()->id,
                                                                    'action'=>'grading'));
            $result .= $this->get_renderer()->continue_button($url);
            $result .= $this->view_footer();
        } else if ($zipfile = $this->pack_files($filesforzipping)) {
            \mod_fastassignment\event\all_submissions_downloaded::create_from_assign($this)->trigger();
    ...
    public function add_to_log($action = '', $info = '', $url='', $return = false) {
        ...
        if ($return) {
            ...
            debugging('The mod_fastassignment add_to_log() function is now deprecated.', DEBUG_DEVELOPER);
            return $args;
        }
     ...
     *
     * @return fastassignment_renderer
     */
    public function get_renderer() {
        ...
        $this->output = $PAGE->get_renderer('mod_fastassignment', null, RENDERER_TARGET_GENERAL);
        return $this->output;
    }
    ...
    protected function get_submission($submissionid) {
        ...
        return $DB->get_record('fastassignment_submission', $params, '*', MUST_EXIST);
    }
    ...
    public function get_user_flags($userid, $create) {
        ...
        $flags = $DB->get_record('fastassignment_user_flags', $params);
        ...
        if ($create) {
            ...
            $fid = $DB->insert_record('fastassignment_user_flags', $flags);
            $flags->id = $fid;
            return $flags;
        }
        return false;
    ...
    public function get_user_grade($userid, $create, $attemptnumber=-1) {
        ...
        $grades = $DB->get_records('fastassignment_grades', $params, 'attemptnumber DESC', '*', 0, 1);
        ...
        if ($create) {
            ...
            $gid = $DB->insert_record('fastassignment_grades', $grade);
            $grade->id = $gid;
            return $grade;
        }
        return false;
    ...
    protected function get_grade($gradeid) {
        ...
        return $DB->get_record('fastassignment_grades', $params, '*', MUST_EXIST);
    }
    ...
    protected function view_single_grading_panel($args) {
        ...
        require_capability('mod/fastassignment:grade', $this->context);
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new fastassignment_submission_status_compact($instance->allowsubmissionsfromdate,
                                                                     ...
                                                                     $this->get_course_module()->id,
                                                                     $this->get_course()->id,
                                                                     fastassignment_submission_status::GRADER_VIEW,
                                                                     $showedit,
                                                                     false,
                                                                     $viewfullnames,
                                                                     $extensionduedate,
        ...
        if ($attemptnumber != -1 && ($attemptnumber + 1) != count($allsubmissions)) {
            ...
            $message = get_string('editingpreviousfeedbackwarning', 'fastassignment', $params);
        ...
        $mform = new mod_fastassignment_grade_form(null,
                                           $formparams,
                                           'post',
                                           '',
        ...
        $o .= $this->get_renderer()->render(new fastassignment_form('gradingform', $mform));

        if (count($allsubmissions) > 1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new fastassignment_attempt_history_chooser($allsubmissions,
                                                          $allgrades,
                                                          $this->get_course_module()->id,
                                                          $userid);
        ...
        \mod_fastassignment\event\grading_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_single_grade_page($mform) {
        ...
        $header = new fastassignment_header($instance,
                                    $this->get_context(),
                                    false,
                                    $this->get_course_module()->id,
                                    get_string('grading', 'fastassignment'));
        $o .= $this->get_renderer()->render($header);
        ...
        if ($user) {
            $this->update_effective_access($userid);
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->get_context());
            $usersummary = new fastassignment_user_summary($user,
                                                   $this->get_course()->id,
                                                   $viewfullnames,
                                                   $this->is_blind_marking(),
        ...
        if ($this->can_view_submission($userid)) {
            ...
            $submissionstatus = new fastassignment_submission_status($instance->allowsubmissionsfromdate,
                                                             $instance->alwaysshowdescription,
                                                             $submission,
                                                             $instance->teamsubmission,
                                                             $teamsubmission,
        ...
        if (count($allsubmissions) > 1 && $attemptnumber == -1) {
            $allgrades = $this->get_all_grades($userid);
            $history = new fastassignment_attempt_history($allsubmissions,
                                                  $allgrades,
                                                  $this->get_submission_plugins(),
    ...
    protected function view_remove_submission_confirm() {
        ...
        $urlparams = array('id' => $this->get_course_module()->id,
                           ...
                           'userid' => $userid,
                           'sesskey' => sesskey());
        $confirmurl = new moodle_url('/mod/fastassignment/view.php', $urlparams);

        $urlparams = array('id' => $this->get_course_module()->id,
                           'action' => 'view');
        $cancelurl = new moodle_url('/mod/fastassignment/view.php', $urlparams);

        if ($userid == $USER->id) {
            $confirmstr = get_string('removesubmissionconfirm', 'fastassignment');
        } else {
            $name = $this->fullname($user);
            $confirmstr = get_string('removesubmissionconfirmforstudent', 'fastassignment', $name);
        }
        ...
        \mod_fastassignment\event\remove_submission_form_viewed::create_from_user($this, $user)->trigger();
    ...
    protected function view_reveal_identities_confirm() {
        require_capability('mod/fastassignment:revealidentities', $this->get_context());
        ...
        $o .= $this->get_renderer()->confirm(get_string('revealidentitiesconfirm', 'fastassignment'),
                                             $confirmurl,
                                             $cancelurl);
        $o .= $this->view_footer();

        \mod_fastassignment\event\reveal_identities_confirmation_page_viewed::create_from_assign($this)->trigger();
    ...
    protected function view_return_links() {
        ...
        $url = new moodle_url('/mod/fastassignment/view.php', $params);
    ...
    protected function view_grading_table() {
        ...
        require_once($CFG->dirroot . '/mod/fastassignment/gradingoptionsform.php');
        require_once($CFG->dirroot . '/mod/fastassignment/quickgradingform.php');
        require_once($CFG->dirroot . '/mod/fastassignment/gradingbatchoperationsform.php');
        $o = '';
        ...
        if (has_capability('gradereport/grader:view', $this->get_course_context()) &&
            ...
            $links[$gradebookurl] = get_string('viewgradebook', 'fastassignment');
        }
        if ($this->is_any_submission_plugin_enabled() && $this->count_submissions()) {
            $downloadurl = '/mod/fastassignment/view.php?id=' . $cmid . '&action=downloadall';
            $links[$downloadurl] = get_string('downloadall', 'fastassignment');
        }
        if ($this->is_blind_marking() &&
                has_capability('mod/fastassignment:revealidentities', $this->get_context())) {
            $revealidentitiesurl = '/mod/fastassignment/view.php?id=' . $cmid . '&action=revealidentities';
            $links[$revealidentitiesurl] = get_string('revealidentities', 'fastassignment');
        }
        foreach ($this->get_feedback_plugins() as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                foreach ($plugin->get_grading_actions() as $action => $description) {
                    $url = '/mod/fastassignment/view.php' .
                           '?id=' .  $cmid .
                           '&plugin=' . $plugin->get_type() .
                           '&pluginsubtype=fastassignfeedback' .
        ...
        $gradingactions->set_label(get_string('choosegradingaction', 'fastassignment'));

        $gradingmanager = get_grading_manager($this->get_context(), 'mod_fastassignment', 'submissions');

        $perpage = $this->get_fastassignment_perpage();
        $filter = get_user_preferences('fastassignment_filter', '');
        $markerfilter = get_user_preferences('fastassignment_markerfilter', '');
        $workflowfilter = get_user_preferences('fastassignment_workflowfilter', '');
        $controller = $gradingmanager->get_active_controller();
        $showquickgrading = empty($controller) && $this->can_grade();
        $quickgrading = get_user_preferences('fastassignment_quickgrading', false);
        $showonlyactiveenrolopt = has_capability('moodle/course:viewsuspendedusers', $this->context);
        $downloadasfolders = get_user_preferences('fastassignment_downloadasfolders', 1);

        $markingallocation = $this->get_instance()->markingworkflow &&
            $this->get_instance()->markingallocation &&
            has_capability('mod/fastassignment:manageallocations', $this->context);
        ...
        if ($markingallocation) {
            ...
            $markers = get_enrolled_users($this->context, 'mod/fastassignment:grade', 0, 'u.*', $sort);
            $markingallocationoptions[''] = get_string('filternone', 'fastassignment');
            $markingallocationoptions[FASTASSIGN_MARKER_FILTER_NO_MARKER] = get_string('markerfilternomarker', 'fastassignment');
        ...
        $gradingoptionsform = new mod_fastassignment_grading_options_form(null,
        ...
        $gradingbatchoperationsform = new mod_fastassignment_grading_batch_operations_form(null,
                                                                                   $batchformparams,
                                                                                   'post',
                                                                                   '',
                                                                                   $classoptions);
        ...
        $currenturl = $CFG->wwwroot .
                      '/mod/fastassignment/view.php?id=' .
                      $this->get_course_module()->id .
                      '&action=grading';
        ...
        if ($this->is_blind_marking() && has_capability('mod/fastassignment:viewblinddetails', $this->get_context())) {
            $o .= $this->get_renderer()->notification(get_string('blindmarkingenabledwarning', 'fastassignment'), 'notifymessage');
        }

        // Load and print the table of submissions.
        if ($showquickgrading && $quickgrading) {
            $gradingtable = new fastassignment_grading_table($this, $perpage, $filter, 0, true);
            ...
            $quickformparams = array('cm'=>$this->get_course_module()->id,
                                     ...
                                     'page' => $page);
            $quickgradingform = new mod_fastassignment_quick_grading_form(null, $quickformparams);

            $o .= $this->get_renderer()->render(new fastassignment_form('quickgradingform', $quickgradingform));
        } else {
            $gradingtable = new fastassignment_grading_table($this, $perpage, $filter, 0, false);
        ...
        if ($this->can_grade()) {
            ...
            $SESSION->mod_fastassignment_useridlist[$this->get_useridlist_key()] = $useridlist;
        }
        ...
        if (count($users) != 0 && $this->can_grade()) {
            ...
            $assignform = new fastassignment_form('gradingbatchoperationsform', $gradingbatchoperationsform);
            $o .= $this->get_renderer()->render($assignform);
        }
        $assignform = new fastassignment_form('gradingoptionsform',
                                      $gradingoptionsform,
                                      'M.mod_fastassignment.init_grading_options');
        $o .= $this->get_renderer()->render($assignform);
        return $o;
    }
    ...
    protected function view_grader() {
        ...
        $args = [
            ...
            'subpage' => get_string('grading', 'fastassignment')
        ];
        ...
        \mod_fastassignment\event\grading_table_viewed::create_from_assign($this)->trigger();
    ...
    protected function view_notices($title, $notices) {
        ...
        $url = new moodle_url('/mod/fastassignment/view.php', array('id'=>$this->get_course_module()->id, 'action'=>'view'));
    ...
    public function fullname($user) {
        if ($this->is_blind_marking()) {
            $hasviewblind = has_capability('mod/fastassignment:viewblinddetails', $this->get_context());
            ...
            if ($hasviewblind) {
                return get_string('participant', 'fastassignment') . ' ' . $uniqueid . ' (' .
    ...
    protected function view_edit_submission_page($mform, $notices) {
        ...
        $o = '';
        require_once($CFG->dirroot . '/mod/fastassignment/submission_form.php');
        // Need submit permission to submit an assignment.
        $userid = optional_param('userid', $USER->id, PARAM_INT);
        $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);
        ...
        if ($userid == $USER->id) {
            ...
            require_capability('mod/fastassignment:submit', $this->context);
            $title = get_string('editsubmission', 'fastassignment');
        } else {
            ...
            $name = $this->fullname($user);
            $title = get_string('editsubmissionother', 'fastassignment', $name);
        }

        if (!$this->submissions_open($userid)) {
            $message = array(get_string('submissionsclosed', 'fastassignment'));
            return $this->view_notices($title, $message);
        }

        $postfix = '';
        if ($this->has_visible_attachments()) {
            $postfix = $this->render_area_files('mod_fastassignment', FASTASSIGN_INTROATTACHMENT_FILEAREA, 0);
        }
        $o .= $this->get_renderer()->render(new fastassignment_header($this->get_instance(),
                                                      $this->get_context(),
                                                      $this->show_intro(),
        ...
        $data->userid = $userid;
        if (!$mform) {
            $mform = new mod_fastassignment_submission_form(null, array($this, $data));
        }
        ...
        $o .= $this->get_renderer()->render(new fastassignment_form('editsubmissionform', $mform));

        $o .= $this->view_footer();

        \mod_fastassignment\event\submission_form_viewed::create_from_user($this, $user)->trigger();
    ...
    public function can_view_submission($userid) {
        ...
        if ($userid == $USER->id && has_capability('mod/fastassignment:submit', $this->context)) {
            return true;
        }
        return false;
    }
    ...
    protected function process_grading_batch_operation(& $mform) {
        ...
        $mform = new mod_fastassignment_grading_batch_operations_form(null,
                                                              $batchformparams,
                                                              'post',
                                                              '',
                                                              $formclasses);
    ...
    protected function view_batch_set_workflow_state($mform) {
        ...
        require_once($CFG->dirroot . '/mod/fastassignment/batchsetmarkingworkflowstateform.php');
        ...
        foreach ($userlist as $userid) {
            if ($usercount >= 5) {
                $usershtml .= get_string('moreusers', 'fastassignment', count($userlist) - 5);
                break;
            }
            $user = $DB->get_record('user', array('id'=>$userid), '*', MUST_EXIST);

            $usershtml .= $this->get_renderer()->render(new fastassignment_user_summary($user,
                                                                $this->get_course()->id,
                                                                $viewfullnames,
                                                                $this->is_blind_marking(),
                                                                $this->get_uniqueid_for_user($user->id),
        ...
        $mform = new mod_fastassignment_batch_set_marking_workflow_state_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new fastassignment_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingworkflowstate', 'fastassignment'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new fastassignment_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_fastassignment\event\batch_set_workflow_state_viewed::create_from_assign($this)->trigger();
    ...
    public function view_batch_markingallocation($mform) {
        ...
        require_once($CFG->dirroot . '/mod/fastassignment/batchsetallocatedmarkerform.php');
        ...
        list($sort, $params) = users_order_by_sql('u');
        // Only enrolled users could be assigned as potential markers.
        $markers = get_enrolled_users($this->get_context(), 'mod/fastassignment:grade', 0, 'u.*', $sort);
        $markerlist = array();
        foreach ($markers as $marker) {
        ...
        $mform = new mod_fastassignment_batch_set_allocatedmarker_form(null, $formparams);
        $mform->set_data($formdata);    // Initialises the hidden elements.
        $header = new fastassignment_header($this->get_instance(),
            $this->get_context(),
            $this->show_intro(),
            $this->get_course_module()->id,
            get_string('setmarkingallocation', 'fastassignment'));
        $o .= $this->get_renderer()->render($header);
        $o .= $this->get_renderer()->render(new fastassignment_form('setworkflowstate', $mform));
        $o .= $this->view_footer();

        \mod_fastassignment\event\batch_set_marker_allocation_viewed::create_from_assign($this)->trigger();
    ...
    protected function check_submit_for_grading($mform) {
        ...
        require_once($CFG->dirroot . '/mod/fastassignment/submissionconfirmform.php');
        ...
        if ($mform == null) {
            $mform = new mod_fastassignment_confirm_submission_form(null, array($requiresubmissionstatement,
                                                                        $submissionstatement,
                                                                        $this->get_course_module()->id,
                                                                        $data));
        }
        ...
        $o .= $this->get_renderer()->render(new fastassignment_header($this->get_instance(),
                                                              ...
                                                              $this->show_intro(),
                                                              $this->get_course_module()->id,
                                                              get_string('confirmsubmissionheading', 'fastassignment')));
        $submitforgradingpage = new fastassignment_submit_for_grading_page($notifications,
                                                                   $this->get_course_module()->id,
                                                                   $mform);
        $o .= $this->get_renderer()->render($submitforgradingpage);
        $o .= $this->view_footer();

        \mod_fastassignment\event\submission_confirmation_form_viewed::create_from_assign($this)->trigger();

        return $o;
    }

    /**
     * Creates an fastassignment_submission_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @param bool $showlinks return plain text or links to the profile
     * @return fastassignment_submission_status renderable object
     */
    public function get_fastassignment_submission_status_renderable($user, $showlinks) {
        global $PAGE;
        ...
        $gradingmanager = get_grading_manager($this->context, 'mod_fastassignment', 'submissions');
        ...
        $submissionstatus = new fastassignment_submission_status($instance->allowsubmissionsfromdate,
                                                          ...
                                                          $this->get_course_module()->id,
                                                          $this->get_course()->id,
                                                          fastassignment_submission_status::STUDENT_VIEW,
                                                          $showedit,
                                                          $showsubmit,
                                                          $viewfullnames,
                                                          $extensionduedate,
    ...
    /**
     * Creates an fastassignment_feedback_status renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return fastassignment_feedback_status renderable object
     */
    public function get_fastassignment_feedback_status_renderable($user) {
        global $CFG, $DB, $PAGE;
        ...
        if ($this->get_instance()->markingworkflow && $gradingstatus != FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED) {
            $emptyplugins = true; // Don't show feedback plugins until released either.
        }

        $cangrade = has_capability('mod/fastassignment:grade', $this->get_context());
        ...
        if (($hasgrade || !$emptyplugins) && $gradevisible) {
            ...
            if ($grade) {
                \mod_fastassignment\event\feedback_viewed::create_from_grade($this, $grade)->trigger();
            }
            $feedbackstatus = new fastassignment_feedback_status($gradefordisplay,
                                                  $gradeddate,
                                                  $grader,
                                                  $this->get_feedback_plugins(),
                                                  $grade,
            ...
            $showgradername = (
                    has_capability('mod/fastassignment:showhiddengrader', $this->context) or
    ...
    /**
     * Creates an fastassignment_attempt_history renderable.
     *
     * @param stdClass $user the user to get the report for
     * @return fastassignment_attempt_history renderable object
     */
    public function get_fastassignment_attempt_history_renderable($user) {
    ...
    public function view_student_summary($user, $showlinks) {
        ...
        if ($this->can_view_submission($user->id)) {
            ...
            if (has_capability('mod/fastassignment:submit', $this->get_context(), $user, false)) {
                $submissionstatus = $this->get_fastassignment_submission_status_renderable($user, $showlinks);
                $o .= $this->get_renderer()->render($submissionstatus);
            }

            // If there is a visible grade, show the feedback.
            $feedbackstatus = $this->get_fastassignment_feedback_status_renderable($user);
            if ($feedbackstatus) {
                $o .= $this->get_renderer()->render($feedbackstatus);
            }

            // If there is more than one submission, show the history.
            $history = $this->get_fastassignment_attempt_history_renderable($user);
    ...
    protected function show_submit_button($submission = null, $teamsubmission = null, $userid = null) {
        if ($teamsubmission) {
            if ($teamsubmission->status === FASTASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                // The assignment submission has been completed.
                return false;
            } else if ($this->submission_empty($teamsubmission)) {
                // There is nothing to submit yet.
                return false;
            } else if ($submission && $submission->status === FASTASSIGN_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if ($submission->status === FASTASSIGN_SUBMISSION_STATUS_SUBMITTED) {
                // The assignment submission has been completed.
                return false;
    ...
    protected function get_all_grades($userid) {
        ...
        $grades = $DB->get_records('fastassignment_grades', $params, 'attemptnumber ASC');
        ...
        // Show the grader's identity if 'Hide Grader' is disabled or has the 'Show Hidden Grader' capability.
        $showgradername = (
            has_capability('mod/fastassignment:showhiddengrader', $this->context, $userid) or
            !$this->is_hidden_grader()
        );
    ...
    public function get_all_submissions($userid) {
        ...
        $submissions = $DB->get_records('fastassignment_submission', $params, 'attemptnumber ASC');

        return $submissions;
    }

    /**
     * Creates an fastassignment_grading_summary renderable.
     *
     * @param mixed $activitygroup int|null the group for calculating the grading summary (if null the function will determine it)
     * @return fastassignment_grading_summary renderable object
     */
    public function get_fastassignment_grading_summary_renderable($activitygroup = null) {

        $instance = $this->get_default_instance(); // Grading summary requires the raw dates, regardless of relativedates mode.
        $cm = $this->get_course_module();
        $course = $this->get_course();

        $draft = FASTASSIGN_SUBMISSION_STATUS_DRAFT;
        $submitted = FASTASSIGN_SUBMISSION_STATUS_SUBMITTED;
        ...
        if ($instance->teamsubmission) {
            $warnofungroupedusers = fastassignment_grading_summary::WARN_GROUPS_NO;
            ...
            if (count($defaultteammembers) > 0) {
                if ($instance->preventsubmissionnotingroup) {
                    $warnofungroupedusers = fastassignment_grading_summary::WARN_GROUPS_REQUIRED;
                } else {
                    $warnofungroupedusers = fastassignment_grading_summary::WARN_GROUPS_OPTIONAL;
                }
            }

            $summary = new fastassignment_grading_summary(
            ...
            $summary = new fastassignment_grading_summary(
                ...
                $this->count_submissions_need_grading($activitygroup),
                $instance->teamsubmission,
                fastassignment_grading_summary::WARN_GROUPS_NO,
                $course->relativedatesmode,
                $course->startdate,
                $this->can_grade(),
                $isvisible
    ...
    private function  get_duedate($activitygroup = null) {
        ...
        if ($this->can_view_grades()) {
            ...
            $groupoverride = $DB->get_record('fastassignment_overrides', $params);
    ...
    protected function view_submission_page() {
        ...
        $o .= $this->get_renderer()->render(new fastassignment_header($instance,
                                                      $this->get_context(),
                                                      $this->show_intro(),
                                                      $this->get_course_module()->id,
                                                      '', '', $postfix));
        ...
        foreach ($plugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                $o .= $this->get_renderer()->render(new fastassignment_plugin_header($plugin));
            }
        }

        if ($this->can_view_grades()) {
            // Group selector will only be displayed if necessary.
            $currenturl = new moodle_url('/mod/fastassignment/view.php', array('id' => $this->get_course_module()->id));
            $o .= groups_print_activity_menu($this->get_course_module(), $currenturl->out(), true);

            $summary = $this->get_fastassignment_grading_summary_renderable();
        ...
        \mod_fastassignment\event\submission_status_viewed::create_from_assign($this)->trigger();
    ...
    protected function gradebook_item_update($submission=null, $grade=null) {
        ...
        if ($this->get_instance()->markingworkflow && !empty($grade) &&
                $this->get_grading_status($grade->userid) != FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED) {
        ...
        $fastassignment = clone $this->get_instance();
        $fastassignment->cmidnumber = $this->get_course_module()->idnumber;
        // Set fastassignment gradebook feedback plugin status (enabled and visible).
        $fastassignment->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
        return fastassignment_grade_item_update($fastassignment, $gradebookgrade) == GRADE_UPDATE_OK;
    }
    ...
    protected function update_team_submission(stdClass $submission, $userid, $updatetime) {
        ...
        $anysubmitted = false;
        $result = true;
        if ($submission->status != FASTASSIGN_SUBMISSION_STATUS_REOPENED) {
            foreach ($team as $member) {
                ...
                if (!$membersubmission || $membersubmission->status != FASTASSIGN_SUBMISSION_STATUS_SUBMITTED
            ...
            if ($this->get_instance()->requireallteammemberssubmit) {
                ...
                $result = $DB->update_record('fastassignment_submission', $submission);
            } else {
                if ($anysubmitted) {
                    $submission->status = FASTASSIGN_SUBMISSION_STATUS_SUBMITTED;
                } else {
            ...
            foreach ($team as $member) {
                ...
                $membersubmission->status = FASTASSIGN_SUBMISSION_STATUS_REOPENED;
                $result = $DB->update_record('fastassignment_submission', $membersubmission) && $result;
            }
            $result = $DB->update_record('fastassignment_submission', $submission) && $result;
        }
    ...
    public function submissions_open($userid = 0,
        ...
        if ($submission) {
            ...
            if ($this->get_instance()->submissiondrafts && $submission->status == FASTASSIGN_SUBMISSION_STATUS_SUBMITTED) {
    ...
    public function render_area_files($component, $area, $submissionid) {
        ...
        return $this->get_renderer()->fastassignment_files($this->context, $submissionid, $area, $component);
    ...
    public function can_edit_submission($userid, $graderid = 0) {
        ...
        if ($userid == $graderid) {
            if ($this->submissions_open($userid) &&
                    has_capability('mod/fastassignment:submit', $this->context, $graderid)) {
        ...
        if (!has_capability('mod/fastassignment:editothersubmission', $this->context, $graderid)) {
            return false;
        }
    ...
    protected function get_graders($userid) {
        ...
        $potentialgraders = get_enrolled_users($this->context, "mod/fastassignment:grade", null, 'u.*', null, null, null, true);
    ...
    protected function get_notifiable_users($userid) {
        ...
        $potentialusers = get_enrolled_users($this->context, "mod/fastassignment:receivegradernotifications",
    ...
    protected static function format_notification_message_text($messagetype,
        ...
        $posttext .= '---------------------------------------------------------------------' . "\n";
        $posttext .= get_string($messagetype . 'text', 'fastassignment', $info)."\n";
        $posttext .= "\n---------------------------------------------------------------------\n";
        return $posttext;
    }
    ...
    protected static function format_notification_message_html($messagetype,
        ...
        $posthtml  = '<p><font face="sans-serif">' .
                     ...
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/fastassignment/index.php?id=' . $course->id . '">' .
                     $modulename .
                     '</a> ->' .
                     '<a href="' . $CFG->wwwroot . '/mod/fastassignment/view.php?id=' . $coursemodule->id . '">' .
                     format_string($assignmentname, true, $formatparams) .
                     '</a></font></p>';
        $posthtml .= '<hr /><font face="sans-serif">';
        $posthtml .= '<p>' . get_string($messagetype . 'html', 'fastassignment', $info) . '</p>';
        $posthtml .= '</font><hr />';
        return $posthtml;
    }
    ...
    public static function send_assignment_notification($userfrom,
        ...
        if ($blindmarking) {
            ...
            $info->username = get_string('participant', 'fastassignment') . ' ' . $uniqueidforuser;
            $userfrom->firstname = get_string('participant', 'fastassignment');
        ...
        $info->url = $CFG->wwwroot.'/mod/fastassignment/view.php?id='.$coursemodule->id;
        $info->timeupdated = userdate($updatetime, get_string('strftimerecentfull'));

        $postsubject = get_string($messagetype . 'small', 'fastassignment', $info);
        ...
        $eventdata->modulename       = 'fastassignment';
        ...
        $eventdata->component       = 'mod_fastassignment';
    ...
    public function submit_for_grading($data, $notices) {
        ...
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('submissionsclosed', 'fastassignment');
            return false;
        }
        ...
        if ($submission->status != FASTASSIGN_SUBMISSION_STATUS_SUBMITTED) {
            ...
            if (!empty($data->submissionstatement) && $USER->id == $userid) {
                \mod_fastassignment\event\statement_accepted::create_from_submission($this, $submission)->trigger();
            }
            $this->notify_graders($submission);
            $this->notify_student_submission_receipt($submission);

            \mod_fastassignment\event\assessable_submitted::create_from_submission($this, $submission, false)->trigger();
    ...
    public function save_user_extension($userid, $extensionduedate) {
        ...
        // Need submit permission to submit an assignment.
        require_capability('mod/fastassignment:grantextension', $this->context);

        if (!is_enrolled($this->get_course_context(), $userid)) {
            return false;
        }
        if (!has_capability('mod/fastassignment:submit', $this->context, $userid)) {
            return false;
        }
        ...
        if ($result) {
            \mod_fastassignment\event\extension_granted::create_from_assign($this, $userid)->trigger();
        }
        return $result;
    }
    ...
    protected function process_save_quick_grades() {
        ...
        $controller = $gradingmanager->get_active_controller();
        if (!empty($controller)) {
            $message = get_string('errorquickgradingvsadvancedgrading', 'fastassignment');
        ...
        if (empty($users)) {
            $message = get_string('nousersselected', 'fastassignment');
            $this->set_error_message($message);
            return $message;
        }
        ...
        $sql = 'SELECT u.id AS userid, g.grade AS grade, g.timemodified AS lastmodified,
             ...
             LEFT JOIN {fastassignment_user_flags} uf ON uf.assignment = g.assignment AND uf.userid = g.userid
                 WHERE u.id ' . $userids;
                ...
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if ($plugin->is_quickgrading_modified($modified->userid, $grade)) {
                        if ((int)$current->lastmodified > (int)$modified->lastmodified) {
                            $message = get_string('errorrecordmodified', 'fastassignment');
                            $this->set_error_message($message);
                            return $message;
                        } else {
                            $modifiedusers[$modified->userid] = $modified;
        ...
        foreach ($modifiedusers as $userid => $modified) {
            ...
            foreach ($this->feedbackplugins as $plugin) {
                if ($plugin->is_visible() && $plugin->is_enabled() && $plugin->supports_quickgrading()) {
                    ...
                    if (('fastassignfeedback_' . $plugin->get_type()) == $gradebookplugin) {
            ...
            if ($workflowstatemodified || $allocatedmarkermodified) {
                if ($this->update_user_flags($flags) && $workflowstatemodified) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_fastassignment\event\workflow_state_updated::create_from_user($this, $user, $flags->workflowstate)->trigger();
                }
            }
            ...
            if ($CFG->enableoutcomes) {
                ...
                if (count($data) > 0) {
                    grade_update_outcomes('mod/fastassignment',
                                          $this->course->id,
                                          'mod',
                                          'fastassignment',
                                          $this->get_instance()->id,
        ...
        return get_string('quickgradingchangessaved', 'fastassignment');
    }
    ...
    public function reveal_identities() {
        ...
        require_capability('mod/fastassignment:revealidentities', $this->context);
        ...
        $adminconfig = $this->get_admin_config();
        $gradebookplugin = $adminconfig->feedback_plugin_for_gradebook;
        $gradebookplugin = str_replace('fastassignfeedback_', '', $gradebookplugin);
        $grades = $DB->get_records('fastassignment_grades', array('assignment'=>$this->get_instance()->id));
        ...
        \mod_fastassignment\event\identities_revealed::create_from_assign($this)->trigger();
    }
    ...
    protected function process_save_grading_options() {
        ...
        $gradingoptionsparams = array('cm'=>$this->get_course_module()->id,
                                      ...
                                      'showonlyactiveenrol' => $this->show_only_active_users(),
                                      'downloadasfolders' => get_user_preferences('fastassignment_downloadasfolders', 1));
        $mform = new mod_fastassignment_grading_options_form(null, $gradingoptionsparams);
        if ($formdata = $mform->get_data()) {
            set_user_preference('fastassignment_perpage', $formdata->perpage);
            if (isset($formdata->filter)) {
                set_user_preference('fastassignment_filter', $formdata->filter);
            }
            if (isset($formdata->markerfilter)) {
                set_user_preference('fastassignment_markerfilter', $formdata->markerfilter);
            }
            if (isset($formdata->workflowfilter)) {
                set_user_preference('fastassignment_workflowfilter', $formdata->workflowfilter);
            }
            if ($showquickgrading) {
                set_user_preference('fastassignment_quickgrading', isset($formdata->quickgrading));
            }
            if (isset($formdata->downloadasfolders)) {
                set_user_preference('fastassignment_downloadasfolders', 1); // Enabled.
            } else {
                set_user_preference('fastassignment_downloadasfolders', 0); // Disabled.
            }
    ...
    public function format_grade_for_log(stdClass $grade) {
        ...
        $info = get_string('gradestudent', 'fastassignment', array('id'=>$user->id, 'fullname'=>fullname($user)));
        if ($grade->grade != '') {
            $info .= get_string('grade') . ': ' . $this->display_grade($grade->grade, false) . '. ';
        } else {
            $info .= get_string('nograde', 'fastassignment');
        }
        return $info;
    }
    ...
    public function format_submission_for_log(stdClass $submission) {
        ...
        if ($submission->userid) {
            ...
            if ($group) {
                ...
                $name = get_string('defaultteam', 'fastassignment');
            }
        }
        $status = get_string('submissionstatus_' . $submission->status, 'fastassignment');
        $params = array('id'=>$submission->userid, 'fullname'=>$name, 'status'=>$status);
        $info .= get_string('submissionlog', 'fastassignment', $params) . ' <br>';
    ...
    public function copy_previous_attempt(&$notices) {
        ...
        if (!$submission || $submission->status != FASTASSIGN_SUBMISSION_STATUS_REOPENED) {
            $notices[] = get_string('submissionnotcopiedinvalidstatus', 'fastassignment');
            return false;
        }
        $flags = $this->get_user_flags($USER->id, false);
        ...
        if ($flags && $flags->locked) {
            $notices[] = get_string('submissionslocked', 'fastassignment');
            return false;
        }
        ...
        \mod_fastassignment\event\submission_duplicated::create_from_submission($this, $submission)->trigger();
        ...
        if (!$instance->submissiondrafts) {
            ...
            \mod_fastassignment\event\assessable_submitted::create_from_submission($this, $submission, true)->trigger();
        }
        return true;
    }
    ...
    public function save_submission(stdClass $data, & $notices) {
        ...
        if ($this->new_submission_empty($data)) {
            $notices[] = get_string('submissionempty', 'mod_fastassignment');
            return false;
        }
        ...
        if (isset($data->lastmodified) && ($submission->timemodified > $data->lastmodified)) {
            ...
            if ($submission->status !== FASTASSIGN_SUBMISSION_STATUS_NEW) {
                $notices[] = $instance->teamsubmission ? get_string('submissionmodifiedgroup', 'mod_fastassignment')
                                                       : get_string('submissionmodified', 'mod_fastassignment');
                return false;
            }
        }
        ...
        // Get the flags to check if it is locked.
        if ($flags && $flags->locked) {
            print_error('submissionslocked', 'fastassignment');
            return true;
        }
    ...
    protected function process_save_submission(&$mform, &$notices) {
        ...
        require_sesskey();
        if (!$this->submissions_open($userid)) {
            $notices[] = get_string('duedatereached', 'fastassignment');
            return false;
        }
        $instance = $this->get_instance();
    ...
    public function add_grade_form_elements(MoodleQuickForm $mform, stdClass $data, $params) {
        ...
        if ($gradinginstance) {
            ...
            if ($this->get_instance()->grade > 0) {
                $name = get_string('gradeoutof', 'fastassignment', $this->get_instance()->grade);
                if (!$gradingdisabled) {

                    // Add Automatic evaluation for teachers
                    global $PAGE, $DB, $cm;

                    $PAGE->requires->jquery();
                    $PAGE->requires->js( new moodle_url("/mod/fastassignment/api_autoeval.js?v=94") );
                    
                    $gradingelement = $mform->addElement('text', 'grade', $name);
                    $mform->addElement('html', '<div class="validation"></div>');
                    $mform->addElement('html', '<div id="loader" style="display: none; margin-left: 380px;"><img src="pix/loader.gif"   alt="Loader" width="51"> Loading...</div>');
                    
                    $mform->addElement('button', 'automatedevaluation', get_string("automatedevaluation", 'fastassignment'));
                    $mform->addElement('html', '<div id="apimessage" style="margin-left: 320px;"></div>');
                    
                    
                    $mform->addHelpButton('grade', 'gradeoutofhelp', 'fastassignment');
                    $mform->setType('grade', PARAM_RAW);
                } else {
                    $strgradelocked = get_string('gradelocked', 'fastassignment');
                    $mform->addElement('static', 'gradedisabled', $name, $strgradelocked);
                    $mform->addHelpButton('gradedisabled', 'gradeoutofhelp', 'fastassignment');
                }
            } else {
        ...
        if ($this->get_instance()->markingworkflow) {
            ...
            $options = array('' => get_string('markingworkflowstatenotmarked', 'fastassignment')) + $states;
            $mform->addElement('select', 'workflowstate', get_string('markingworkflowstate', 'fastassignment'), $options);
            $mform->addHelpButton('workflowstate', 'markingworkflowstate', 'fastassignment');
            $gradingstatus = $this->get_grading_status($userid);
            if ($gradingstatus != FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED) {
                if ($grade->grade && $grade->grade != -1) {
                    $assigngradestring = html_writer::span(grade_floatval($grade->grade), 'currentgrade');
                    $label = get_string('currentassigngrade', 'fastassignment');
        ...
        if ($this->get_instance()->markingworkflow &&
            ...
            // Only enrolled users could be assigned as potential markers.
            $markers = get_enrolled_users($this->context, 'mod/fastassignment:grade', 0, 'u.*', $sort);
            $markerlist = array('' =>  get_string('choosemarker', 'fastassignment'));
            $viewfullnames = has_capability('moodle/site:viewfullnames', $this->context);
            foreach ($markers as $marker) {
                $markerlist[$marker->id] = fullname($marker, $viewfullnames);
            }
            $mform->addElement('select', 'allocatedmarker', get_string('allocatedmarker', 'fastassignment'), $markerlist);
            $mform->addHelpButton('allocatedmarker', 'allocatedmarker', 'fastassignment');
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', FASTASSIGN_MARKING_WORKFLOW_STATE_INREVIEW);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORRELEASE);
            $mform->disabledIf('allocatedmarker', 'workflowstate', 'eq', FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED);
        }

        $gradestring = '<span class="currentgrade">' . $gradestring . '</span>';
        $mform->addElement('static', 'currentgrade', get_string('currentgrade', 'fastassignment'), $gradestring);

        if (count($useridlist) > 1) {
            $strparams = array('current'=>$rownum+1, 'total'=>count($useridlist));
            $name = get_string('outof', 'fastassignment', $strparams);
            $mform->addElement('static', 'gradingstudent', get_string('gradingstudent', 'fastassignment'), $name);
        }
        ...
        if ($this->get_instance()->teamsubmission) {
            ...
            $mform->addElement('selectyesno', 'applytoall', get_string('applytoteam', 'fastassignment'));
        ...
        if (($attemptnumber == -1 ||
            ...
            $this->get_instance()->attemptreopenmethod != FASTASSIGN_ATTEMPT_REOPEN_METHOD_NONE) {
            $mform->addElement('header', 'attemptsettings', get_string('attemptsettings', 'fastassignment'));
            $attemptreopenmethod = get_string('attemptreopenmethod_' . $this->get_instance()->attemptreopenmethod, 'fastassignment');
            $mform->addElement('static', 'attemptreopenmethod', get_string('attemptreopenmethod', 'fastassignment'), $attemptreopenmethod);
            ...
            if ($maxattempts == FASTASSIGN_UNLIMITED_ATTEMPTS) {
                $maxattempts = get_string('unlimitedattempts', 'fastassignment');
            }
            $mform->addelement('static', 'maxattemptslabel', get_string('maxattempts', 'fastassignment'), $maxattempts);
            $mform->addelement('static', 'attemptnumberlabel', get_string('attemptnumber', 'fastassignment'), $attemptnumber + 1);

            $ismanual = $this->get_instance()->attemptreopenmethod == FASTASSIGN_ATTEMPT_REOPEN_METHOD_MANUAL;
            $issubmission = !empty($submission);
            $isunlimited = $this->get_instance()->maxattempts == FASTASSIGN_UNLIMITED_ATTEMPTS;
            ...
            if ($ismanual && (!$issubmission || $isunlimited || $islessthanmaxattempts)) {
                $mform->addElement('selectyesno', 'addattempt', get_string('addattempt', 'fastassignment'));
                $mform->setDefault('addattempt', 0);
            }
        }
        if (!$gradingpanel) {
            $mform->addElement('selectyesno', 'sendstudentnotifications', get_string('sendstudentnotifications', 'fastassignment'));
        } else {
            $mform->addElement('hidden', 'sendstudentnotifications', get_string('sendstudentnotifications', 'fastassignment'));
        ...
        if (!$cm->uservisible) {
            ...
            $mform->setDefault('sendstudentnotifications', 0);
            if (!$gradingpanel) {
                $mform->disabledIf('sendstudentnotifications', 'workflowstate', 'neq', FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED);
            }
        } else {
        ...
        if (!$gradingpanel) {
            ...
            $buttonarray = array();
            $name = get_string('savechanges', 'fastassignment');
            $buttonarray[] = $mform->createElement('submit', 'savegrade', $name);
            if (!$last) {
                $name = get_string('savenext', 'fastassignment');
            ...
            if ($rownum > 0) {
                $name = get_string('previous', 'fastassignment');
                $buttonarray[] = $mform->createElement('submit', 'nosaveandprevious', $name);
            }

            if (!$last) {
                $name = get_string('nosavebutnext', 'fastassignment');
    ...
    public function remove_submission($userid) {
        ...
        if (!$this->can_edit_submission($userid, $USER->id)) {
            ...
            $message = get_string('usersubmissioncannotberemoved', 'fastassignment', fullname($user));
            $this->set_error_message($message);
            return false;
        }
        ...
        if ($submission->userid != 0) {
            \mod_fastassignment\event\submission_status_updated::create_from_submission($this, $submission)->trigger();
        }
        return true;
    }
    ...
    public function lock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_fastassignment\event\submission_locked::create_from_user($this, $user)->trigger();
        return true;
    }
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                ...
                if ($formdata->sendstudentnotifications && $cm->uservisible &&
                        $state == FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED) {
                    $flags->mailed = 0;
                }

                $gradingdisabled = $this->grading_disabled($userid);

                // Will not apply update if user does not have permission to fastassignment this workflow state.
                if (!$gradingdisabled && $this->update_user_flags($flags)) {
                    ...
                    $fastassignment = clone $this->get_instance();
                    $fastassignment->cmidnumber = $this->get_course_module()->idnumber;
                    // Set fastassignment gradebook feedback plugin status.
                    $fastassignment->gradefeedbackenabled = $this->is_gradebook_feedback_enabled();
                    fastassignment_update_grades($fastassignment, $userid);

                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_fastassignment\event\workflow_state_updated::create_from_user($this, $user, $state)->trigger();
                }
            }
        }
    }
    ...
    protected function process_set_batch_marking_allocation() {
        ...
        if ($formdata = $mform->get_data()) {
            ...
            foreach ($useridlist as $userid) {
                $flags = $this->get_user_flags($userid, true);
                if ($flags->workflowstate == FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORREVIEW ||
                    $flags->workflowstate == FASTASSIGN_MARKING_WORKFLOW_STATE_INREVIEW ||
                    $flags->workflowstate == FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORRELEASE ||
                    $flags->workflowstate == FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED) {
                ...
                if ($this->update_user_flags($flags)) {
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_fastassignment\event\marker_updated::create_from_marker($this, $user, $marker)->trigger();
                }
            }
        }
    }
    ...
    public function unlock_submission($userid) {
        ...
        $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
        \mod_fastassignment\event\submission_unlocked::create_from_user($this, $user)->trigger();
        return true;
    }
    ...
    protected function apply_grade_to_user($formdata, $userid, $attemptnumber) {
        ...
        if (!$gradingdisabled) {
            ...
            if (isset($formdata->workflowstate) || isset($formdata->allocatedmarker)) {
                ...
                if ($this->update_user_flags($flags) &&
                    ...
                    $user = $DB->get_record('user', array('id' => $userid), '*', MUST_EXIST);
                    \mod_fastassignment\event\workflow_state_updated::create_from_user($this, $user, $formdata->workflowstate)->trigger();
                }
            }
        }
        $grade->grader= $USER->id;
        ...
        if ($this->get_instance()->markingworkflow &&
                isset($formdata->workflowstate) &&
                $formdata->workflowstate != FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED) {
    ...
    protected function reopen_submission_if_required($userid, $submission, $addattempt) {
        ...
        $maxattemptsreached = !empty($submission) &&
                              ...
                              $instance->maxattempts != FASTASSIGN_UNLIMITED_ATTEMPTS;
        $shouldreopen = false;
        if ($instance->attemptreopenmethod == FASTASSIGN_ATTEMPT_REOPEN_METHOD_UNTILPASS) {
    ...
    protected function process_save_grade(&$mform) {
        ...
        $userid = optional_param('userid', 0, PARAM_INT);
        if (!$userid) {
            if (empty($SESSION->mod_fastassignment_useridlist[$this->get_useridlist_key($useridlistid)])) {
                ...
                $url = new moodle_url('/mod/fastassignment/view.php', array('id' => $this->get_course_module()->id));
                throw new moodle_exception('useridlistnotcached', 'mod_fastassignment', $url);
            }
            $useridlist = $SESSION->mod_fastassignment_useridlist[$this->get_useridlist_key($useridlistid)];
        } else {
            $useridlist = array($userid);
            $rownum = 0;
        }
    ...
    public static function can_upgrade_assignment($type, $version) {
        $assignment = new fastassignment(null, null, null);
    ...
    protected function add_attempt($userid) {
        ...
        if ($this->get_instance()->attemptreopenmethod == FASTASSIGN_ATTEMPT_REOPEN_METHOD_NONE) {
            return false;
        }
        ...
        if ($this->get_instance()->maxattempts != FASTASSIGN_UNLIMITED_ATTEMPTS &&
        ...
        $newsubmission->status = FASTASSIGN_SUBMISSION_STATUS_REOPENED;
    ...
    public function get_user_grades_for_gradebook($userid) {
        ...
        foreach ($this->feedbackplugins as $plugin) {
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                if (('fastassignfeedback_' . $plugin->get_type()) == $gradebookpluginname) {
    ...
    /**
     * Foreach participant in the course - fastassignment them a random id.
     *
    ...
    public static function allocate_unique_ids($assignid) {
        ...
        $cm = get_coursemodule_from_instance('fastassignment', $assignid, 0, false, MUST_EXIST);
        $context = context_module::instance($cm->id);

        $currentgroup = groups_get_activity_group($cm, true);
        $users = get_enrolled_users($context, "mod/fastassignment:submit", $currentgroup, 'u.id');

        // Shuffle the users.
        shuffle($users);

        foreach ($users as $user) {
            $record = $DB->get_record('fastassignment_user_mapping',
            ...
            if (!$record) {
                ...
                $DB->insert_record('fastassignment_user_mapping', $record);
            }
        }
    }
    ...
    public static function get_uniqueid_for_user_static($assignid, $userid) {
        ...
        if ($record = $DB->get_record('fastassignment_user_mapping', $params, 'id')) {
            return $record->id;
        }
        ...
        return $DB->insert_record('fastassignment_user_mapping', $record);
    }
    ...
    public static function get_user_id_for_uniqueid_static($assignid, $uniqueid) {
        ...
        // Search for a record.
        if ($record = $DB->get_record('fastassignment_user_mapping',
    ...
    public function get_marking_workflow_states_for_current_user() {
        ...
        $states = array();
        if (has_capability('mod/fastassignment:grade', $this->context)) {
            $states[FASTASSIGN_MARKING_WORKFLOW_STATE_INMARKING] = get_string('markingworkflowstateinmarking', 'fastassignment');
            $states[FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORREVIEW] = get_string('markingworkflowstatereadyforreview', 'fastassignment');
        }
        if (has_any_capability(array('mod/fastassignment:reviewgrades',
                                     'mod/fastassignment:managegrades'), $this->context)) {
            $states[FASTASSIGN_MARKING_WORKFLOW_STATE_INREVIEW] = get_string('markingworkflowstateinreview', 'fastassignment');
            $states[FASTASSIGN_MARKING_WORKFLOW_STATE_READYFORRELEASE] = get_string('markingworkflowstatereadyforrelease', 'fastassignment');
        }
        if (has_any_capability(array('mod/fastassignment:releasegrades',
                                     'mod/fastassignment:managegrades'), $this->context)) {
            $states[FASTASSIGN_MARKING_WORKFLOW_STATE_RELEASED] = get_string('markingworkflowstatereleased', 'fastassignment');
        }
    ...
    public function get_grading_status($userid) {
        if ($this->get_instance()->markingworkflow) {
            ...
            return FASTASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED;
        } else {
            $attemptnumber = optional_param('attemptnumber', -1, PARAM_INT);
            $grade = $this->get_user_grade($userid, false, $attemptnumber);

            if (!empty($grade) && $grade->grade !== null && $grade->grade >= 0) {
                return FASTASSIGN_GRADING_STATUS_GRADED;
            } else {
                return FASTASSIGN_GRADING_STATUS_NOT_GRADED;
            }
        }
    }

    /**
     * The id used to uniquily identify the cache for this instance of the fastassignment object.
     *
     * @return string
     */
    ...
    public function set_module_viewed() {
        ...
        $event->add_record_snapshot('fastassignment', $assigninstance);
        $event->trigger();
    }
    ...
    protected function add_grade_notices() {
        if (has_capability('mod/fastassignment:grade', $this->get_context()) && get_config('fastassignment', 'has_rescaled_null_grades_' . $this->get_instance()->id)) {
            $link = new \moodle_url('/mod/fastassignment/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades'));
            \core\notification::warning(get_string('fixrescalednullgrades', 'mod_fastassignment', ['link' => $link->out()]));
        }
    }
    ...
    protected function view_fix_rescaled_null_grades() {
        ...
        require_capability('mod/fastassignment:grade', $this->get_context());

        $instance = $this->get_instance();

        $o .= $this->get_renderer()->render(
            new fastassignment_header(
                $instance,
                $this->get_context(),
                $this->show_intro(),
                $this->get_course_module()->id
        ...
        if ($confirm) {
            ...
            unset_config('has_rescaled_null_grades_' . $instance->id, 'fastassignment');

            // Display the notice.
            $o .= $this->get_renderer()->notification(get_string('fixrescalednullgradesdone', 'fastassignment'), 'notifysuccess');
            $url = new moodle_url(
                '/mod/fastassignment/view.php',
                array(
            ...
            $continue = new \moodle_url('/mod/fastassignment/view.php', array('id' => $this->get_course_module()->id, 'action' => 'fixrescalednullgrades', 'confirm' => true, 'sesskey' => sesskey()));
            $cancel = new \moodle_url('/mod/fastassignment/view.php', array('id' => $this->get_course_module()->id));
            $o .= $OUTPUT->confirm(get_string('fixrescalednullgradesconfirm', 'mod_fastassignment'), $continue, $cancel);
        }
    ...
    public function get_marking_allocation_filters($export = false) {
        ...
        if ($export) {
            $allocationfilter = get_user_preferences('fastassignment_markerfilter', '');
            $result = [];
    ...
    public function get_marking_workflow_filters($export = false) {
        ...
        if ($markingworkflow) {
            $notmarked = get_string('markingworkflowstatenotmarked', 'fastassignment');
            $markingworkflowoptions[''] = get_string('filternone', 'fastassignment');
            $markingworkflowoptions[FASTASSIGN_MARKING_WORKFLOW_STATE_NOTMARKED] = $notmarked;
    ...
    public function get_filters() {
        $filterkeys = [
            FASTASSIGN_FILTER_SUBMITTED,
            FASTASSIGN_FILTER_NOT_SUBMITTED,
            FASTASSIGN_FILTER_REQUIRE_GRADING,
            FASTASSIGN_FILTER_GRANTED_EXTENSION
        ];

        $current = get_user_preferences('fastassignment_filter', '');

        $filters = [];
        // First is always "no filter" option.
        array_push($filters, [
            'key' => 'none',
            'name' => get_string('filternone', 'fastassignment'),
            'active' => ($current == '')
        ]);
        ...
        foreach ($filterkeys as $key) {
            array_push($filters, [
                'key' => $key,
                'name' => get_string('filter' . $key, 'fastassignment'),
                'active' => ($current == $key)
            ]);
        }
        return $filters;
...
/**
 * Portfolio caller class for mod_fastassignment.
 *
 * @package   mod_fastassignment
 * @copyright 2012 NetSpot {@link http://www.netspot.com.au}
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_portfolio_caller extends portfolio_module_caller_base {
    ...
    public function __construct($callbackargs) {
        ...
        $this->cm = get_coursemodule_from_id('fastassignment', $this->cmid, 0, false, MUST_EXIST);
    }
    ...
    public function load_data() {
        ...
        if (empty($this->fileid)) {
            if (empty($this->sid) || empty($this->area)) {
                throw new portfolio_caller_exception('invalidfileandsubmissionid', 'mod_fastassignment');
            }

            $submission = $DB->get_record('fastassignment_submission', array('id' => $this->sid));
        } else {
            $submissionid = $DB->get_field('files', 'itemid', array('id' => $this->fileid, 'contextid' => $context->id));
            if ($submissionid) {
                $submission = $DB->get_record('fastassignment_submission', array('id' => $submissionid));
            }
        }
    ...
    public function prepare_package() {
        ...
        if ($this->exporter->get('formatclass') == PORTFOLIO_FORMAT_LEAP2A) {
            ...
            if (count($files) > 1) {
                $baseid = 'fastassignment' . $this->cmid . $this->area;
                $context = context_module::instance($this->cmid);
     ...
     *
     * @return fastassignment_submission_plugin
     */
    ...
    public function check_permissions() {
        $context = context_module::instance($this->cmid);
        return has_capability('mod/fastassignment:exportownsubmission', $context);
    }
    ...
    public static function display_name() {
        return get_string('modulename', 'fastassignment');
    }
 ...
 * @return void
 */
function fastassignment_process_group_deleted_in_course($courseid, $groupid = null) {
    global $DB;
    ...
    if ($groupid) {
        ...
        $sql = "SELECT o.id, o.assignid
                  FROM {fastassignment_overrides} o
                  JOIN {fastassignment} fastassignment ON fastassignment.id = o.assignid
                 WHERE fastassignment.course = :courseid
                   AND o.groupid = :groupid";
    } else {
        // No groupid, we update all orphaned group overrides for all fastassignment in course.
        $sql = "SELECT o.id, o.assignid
                  FROM {fastassignment_overrides} o
                  JOIN {fastassignment} fastassignment ON fastassignment.id = o.assignid
    ...
    $DB->delete_records_list('fastassignment_overrides', 'id', array_keys($records));
}
 ...
 */
function move_group_override_fast($id, $move, $assignid) {
    global $DB;

    // Get the override object.
    if (!$override = $DB->get_record('fastassignment_overrides', array('id' => $id), 'id, sortorder')) {
        return false;
    }
    // Count the number of group overrides.
    $overridecountgroup = $DB->count_records('fastassignment_overrides', array('userid' => null, 'assignid' => $assignid));
    ...
    if ($swapoverride = $DB->get_record('fastassignment_overrides', $params, 'id, sortorder')) {
        ...
        $DB->update_record('fastassignment_overrides', $override);
        $DB->update_record('fastassignment_overrides', $swapoverride);
    }

    reorder_group_overrides_fast($assignid);
    return true;
}
 ...
 */
function reorder_group_overrides_fast($assignid) {
    global $DB;

    $i = 1;
    if ($overrides = $DB->get_records('fastassignment_overrides', array('userid' => null, 'assignid' => $assignid), 'sortorder ASC')) {
        foreach ($overrides as $override) {
            $f = new stdClass();
            $f->id = $override->id;
            $f->sortorder = $i++;
            $DB->update_record('fastassignment_overrides', $f);
