...
/**
 * fastassignment functions
 * @copyright 2012 Paul Charsley
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class mod_fastassignment_external extends external_api {
    ...
    /**
     * Returns grade information from fastassignment_grades for the requested assignment ids
    ...
    public static function get_grades($assignmentids, $since = 0) {
        ...
        $placeholders['modname'] = 'fastassignment';
        ...
        foreach ($cms as $cm) {
            try {
                ...
                $fastassignment->require_view_grades();
            } catch (Exception $e) {
        ...
        if (count ($requestedassignmentids) > 0) {
            ...
            $sql = "SELECT ag.id,
                           ...
                           ag.grade,
                           ag.attemptnumber
                      FROM {fastassignment_grades} ag, {fastassignment_submission} s
    ...
    /**
     * Creates an fastassignment_grades external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.4
     */
    private static function fastassignment_grades() {
        return new external_single_structure(
            array (
    ...
    public static function get_grades_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::fastassignment_grades(), 'list of assignment grade information'),
    ...
    public static function get_assignments($courseids = array(), $capabilities = array(), $includenotenrolledcourses = false) {
        ...
        foreach ($courses as $id => $course) {
            ...
            if ($modules = get_coursemodules_in_course('fastassignment', $courses[$id]->id, $extrafields)) {
                foreach ($modules as $module) {
                    ...
                    $fastassignment = new fastassignment($context, null, null);
                    // Update fastassignment with override information.
                    $fastassignment->update_effective_access($USER->id);

                    // Get configurations for only enabled plugins.
                    $plugins = $fastassignment->get_submission_plugins();
                    $plugins = array_merge($plugins, $fastassignment->get_feedback_plugins());
                    ...
                    $assignment = array(
                        ...
                        'duedate' => $fastassignment->get_instance()->duedate,
                        'allowsubmissionsfromdate' => $fastassignment->get_instance()->allowsubmissionsfromdate,
                        'grade' => $module->grade,
                        'timemodified' => $module->timemodified,
                        'completionsubmit' => $module->completionsubmit,
                        'cutoffdate' => $fastassignment->get_instance()->cutoffdate,
                        'gradingduedate' => $fastassignment->get_instance()->gradingduedate,
                    ...
                    if ($fastassignment->show_intro()) {
                        ...
                        list($assignment['intro'], $assignment['introformat']) =
                            external_format_text($module->intro, $module->introformat, $context->id, 'mod_fastassignment', 'intro', null,
                                $options);
                        $assignment['introfiles'] = external_util::get_area_files($context->id, 'mod_fastassignment', 'intro', false,
                                                                                    false);

                        $assignment['introattachments'] = external_util::get_area_files($context->id, 'mod_fastassignment',
                                                            FASTASSIGN_INTROATTACHMENT_FILEAREA, 0);
                    }
                            ...
                            if (!empty($adminconfig->submissionstatementteamsubmission) && !$module->requireallteammemberssubmit) {
                                list($assignment['submissionstatement'], $assignment['submissionstatementformat']) =
                                    external_format_text($adminconfig->submissionstatementteamsubmission,
                                        FORMAT_MOODLE, $context->id, 'mod_fastassignment', '', 0);
    ...
    /**
     * Creates an fastassignment_plugin_config external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.4
     */
    private static function get_assignments_config_structure() {
        return new external_single_structure(
            array(
                'id' => new external_value(PARAM_INT, 'fastassignment_plugin_config id', VALUE_OPTIONAL),
                'assignment' => new external_value(PARAM_INT, 'assignment id', VALUE_OPTIONAL),
     ...
     */
    private static function get_plugins_data($fastassignment, $assignplugins, $item) {
        global $CFG;
        ...
        foreach ($assignplugins as $assignplugin) {
            ...
            // Subtype is 'fastassignsubmission', type is currently 'file' or 'onlinetext'.
            ...
            foreach ($editorfields as $name => $description) {
                ...
                foreach ($fileareas as $filearea => $name) {
                    list($editorfieldinfo['text'], $editorfieldinfo['format']) = external_format_text(
                        $editorfieldinfo['text'], $editorfieldinfo['format'], $fastassignment->get_context()->id,
    ...
    public static function get_submissions($assignmentids, $status = '', $since = 0, $before = 0) {
        ...
        foreach ($assigns as $fastassignment) {
            $submissions = array();
            $placeholders = array('assignid1' => $fastassignment->get_instance()->id,
                                  'assignid2' => $fastassignment->get_instance()->id);

            $submissionmaxattempt = 'SELECT mxs.userid, MAX(mxs.attemptnumber) AS maxattempt
                                     FROM {fastassignment_submission} mxs
            ...
            $sql = "SELECT mas.id, mas.assignment,mas.userid,".
                   ...
                   "FROM {fastassignment_submission} mas ".
            ...
            if (!empty($submissionrecords)) {
                ...
                foreach ($submissionrecords as $submissionrecord) {
                    $submission = array(
                        ...
                        'plugins' => self::get_plugins_data($fastassignment, $submissionplugins, $submissionrecord),
                        'gradingstatus' => $fastassignment->get_grading_status($submissionrecord->userid)
                    );

                    if ($fastassignment->can_view_submission($submissionrecord->userid)) {
                ...
                $warnings[] = array(
                    'item' => 'module',
                    'itemid' => $fastassignment->get_instance()->id,
                    'warningcode' => '3',
            ...
            $assignments[] = array(
                'assignmentid' => $fastassignment->get_instance()->id,
    ...
    /**
     * Creates an fastassignment_submissions external_single_structure
     *
     * @return external_single_structure
     * @since Moodle 2.5
     */
    ...
    public static function set_user_flags($assignmentid, $userflags = array()) {
        ...
        list($fastassignment, $course, $cm, $context) = self::validate_assign($params['assignmentid']);
        require_capability('mod/fastassignment:grade', $context);
        ...
        foreach ($params['userflags'] as $userflag) {
            ...
            $record = $fastassignment->get_user_flags($userflag['userid'], false);
            if ($record) {
                ...
                if ($fastassignment->update_user_flags($record)) {
                    $result['id'] = $record->id;
                ...
                $record = $fastassignment->get_user_flags($userflag['userid'], true);
    ...
    /**
     * Returns user flag information from fastassignment_user_flags for the requested assignment ids
     * @param int[] $assignmentids
    ...
    public static function get_user_flags($assignmentids) {
        ...
        // Create the query and populate an array of fastassignment_user_flags records from the recordset results.
        if (count ($requestedassignmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedassignmentids, SQL_PARAMS_NAMED);
            ...
            $sql = "SELECT auf.id,auf.assignment,auf.userid,auf.locked,auf.mailed,".
                   ...
                   "FROM {fastassignment_user_flags} auf ".
    ...
    /**
     * Creates an fastassignment_user_flags external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function fastassignment_user_flags() {
        return new external_single_structure(
            array (
                'assignmentid'    => new external_value(PARAM_INT, 'assignment id'),
    ...
    public static function get_user_flags_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::fastassignment_user_flags(), 'list of fastassignment user flag information'),
                'warnings'      => new external_warnings('item is always \'assignment\'',
                    'when errorcode is 3 then itemid is an assignment id. When errorcode is 1, itemid is a course module id',
    ...
    /**
     * Returns user mapping information from fastassignment_user_mapping for the requested assignment ids
     * @param int[] $assignmentids
    ...
    public static function get_user_mappings($assignmentids) {
        ...
        foreach ($cms as $cm) {
            try {
                $context = context_module::instance($cm->id);
                self::validate_context($context);
                require_capability('mod/fastassignment:revealidentities', $context);
            } catch (Exception $e) {
                $requestedassignmentids = array_diff($requestedassignmentids, array($cm->instance));
                $warning = array();
                $warning['item'] = 'assignment';
        ...
        // Create the query and populate an array of fastassignment_user_mapping records from the recordset results.
        if (count ($requestedassignmentids) > 0) {
            $placeholders = array();
            list($inorequalsql, $placeholders) = $DB->get_in_or_equal($requestedassignmentids, SQL_PARAMS_NAMED);

            $sql = "SELECT aum.id,aum.assignment,aum.userid ".
                   "FROM {fastassignment_user_mapping} aum ".
    ...
    /**
     * Creates an fastassignment_user_mappings external_single_structure
     * @return external_single_structure
     * @since  Moodle 2.6
     */
    private static function fastassignment_user_mappings() {
        return new external_single_structure(
            array (
                'assignmentid'    => new external_value(PARAM_INT, 'assignment id'),
    ...
    public static function get_user_mappings_returns() {
        return new external_single_structure(
            array(
                'assignments' => new external_multiple_structure(self::fastassignment_user_mappings(), 'list of fastassignment user mapping data'),
                'warnings'      => new external_warnings('item is always \'assignment\'',
                    'when errorcode is 3 then itemid is an assignment id. When errorcode is 1, itemid is a course module id',
    ...
    public static function submit_grading_form($assignmentid, $userid, $jsonformdata) {
        ...
        if (WS_SERVER) {
            ...
            $data['_qf__mod_fastassignment_grade_form_'.$params['userid']] = 1;
        }
        ...
        $mform = new mod_fastassignment_grade_form(null, $formparams, 'post', '', null, true, $data);
    ...
    public static function save_submission_parameters() {
        global $CFG;
        $instance = new fastassignment(null, null, null);
    ...
    public static function view_grading_table_parameters() {
        return new external_function_parameters(
            array(
                'assignid' => new external_value(PARAM_INT, 'fastassignment instance id')
            )
        );
    }
     ...
     *
     * @param int $assignid the fastassignment instance id
    ...
    public static function view_grading_table($assignid) {
        ...
        list($fastassignment, $course, $cm, $context) = self::validate_assign($params['assignid']);

        $fastassignment->require_view_grades();
        \mod_fastassignment\event\grading_table_viewed::create_from_assign($fastassignment)->trigger();
     ...
     *
     * @param int $assignid fastassignment instance id
     * @return array of warnings and status result
     * @since Moodle 3.1
     */
    public static function view_submission_status($assignid) {
        ...
        \mod_fastassignment\event\submission_status_viewed::create_from_assign($fastassignment)->trigger();
    ...
    public static function get_submission_status($assignid, $userid = 0, $groupid = 0) {
        ...
        if (!$fastassignment->can_view_submission($user->id)) {
            throw new required_capability_exception($context, 'mod/fastassignment:viewgrades', 'nopermission', '');
        }
        ...
        if ($fastassignment->can_view_grades($groupid)) {
            $gradingsummary = $fastassignment->get_fastassignment_grading_summary_renderable($groupid);
        }

        // Retrieve the rest of the renderable objects.
        if (has_capability('mod/fastassignment:submit', $fastassignment->get_context(), $user)) {
            $lastattempt = $fastassignment->get_fastassignment_submission_status_renderable($user, true);
        }

        $feedback = $fastassignment->get_fastassignment_feedback_status_renderable($user);

        $previousattempts = $fastassignment->get_fastassignment_attempt_history_renderable($user);
        ...
        $showgradername = (has_capability('mod/fastassignment:showhiddengrader', $context) or
            !$fastassignment->is_hidden_grader());
        ...
        if ($lastattempt) {
            ...
            if (empty($lastattempt->submission)) {
                ...
                $lastattempt->submission->plugins = self::get_plugins_data($fastassignment, $submissionplugins, $lastattempt->submission);
            }

            if (empty($lastattempt->teamsubmission)) {
                unset($lastattempt->teamsubmission);
            } else {
                $lastattempt->teamsubmission->plugins = self::get_plugins_data($fastassignment, $submissionplugins,
            ...
            $lastattempt->caneditowner = $fastassignment->submissions_open($user->id) && $fastassignment->is_any_submission_plugin_enabled();
        ...
        if ($feedback) {
            if ($feedback->grade) {
                ...
                $feedbackplugins = $fastassignment->get_feedback_plugins();
                $feedback->plugins = self::get_plugins_data($fastassignment, $feedbackplugins, $feedback->grade);
            } else {
        ...
        if ($previousattempts and count($previousattempts->submissions) > 1) {
            ...
            foreach ($previousattempts->submissions as $i => $submission) {
                ...
                if ($submission) {
                    $submission->plugins = self::get_plugins_data($fastassignment, $previousattempts->submissionplugins, $submission);
                ...
                if ($grade) {
                    ...
                    $feedbackplugins = self::get_plugins_data($fastassignment, $previousattempts->feedbackplugins, $grade);
    ...
    public static function list_participants($assignid, $groupid, $filter, $skip,
        ...
        if (groups_group_visible($params['groupid'], $course, $cm)) {
            $participants = $fastassignment->list_participants_with_filter_status_and_group($params['groupid'], $params['tablesort']);
        }
        ...
        foreach ($participants as $record) {
            ...
            if ($match) {
                ...
                if (!$fastassignment->is_blind_marking() && !$params['onlyids']) {
    ...
    /**
     * Returns the description of the results of the mod_fastassignment_external::list_participants() method.
     *
     * @return external_description
     * @since Moodle 3.1
     */
    ...
    public static function get_participant($assignid, $userid, $embeduser) {
        ...
        $participant = $fastassignment->get_participant($params['userid']);

        // Update fastassignment with override information.
        $fastassignment->update_effective_access($params['userid']);
        ...
        $return = array(
            ...
            'blindmarking' => $fastassignment->is_blind_marking(),
            'allowsubmissionsfromdate' => $fastassignment->get_instance($userid)->allowsubmissionsfromdate,
            'duedate' => $fastassignment->get_instance($userid)->duedate,
            'cutoffdate' => $fastassignment->get_instance($userid)->cutoffdate,
            'duedatestr' => userdate($fastassignment->get_instance($userid)->duedate, get_string('strftimedatetime', 'langconfig')),
        );
        ...
        if (!$fastassignment->is_blind_marking() && $embeduser) {
    ...
    /**
     * Utility function for validating an fastassignment.
     *
     * @param int $assignid fastassignment instance id
     * @return array array containing the fastassignment, course, context and course module objects
     * @since  Moodle 3.2
     */
    protected static function validate_assign($assignid) {
        ...
        $fastassignment = $DB->get_record('fastassignment', array('id' => $assignid), 'id', MUST_EXIST);
        list($course, $cm) = get_course_and_cm_from_instance($fastassignment, 'fastassignment');

        $context = context_module::instance($cm->id);
        // Please, note that is not required to check mod/fastassignment:view because is done by validate_context->require_login.
        self::validate_context($context);
        $fastassignment = new fastassignment($context, $cm, $course);

        return array($fastassignment, $course, $cm, $context);
    }
     ...
     * @since Moodle 3.2
     */
    public static function view_fastassignment_parameters() {
        return new external_function_parameters (
            array(
                'assignid' => new external_value(PARAM_INT, 'fastassignment instance id'),
            )
    ...
    public static function view_assign($assignid) {
        ...
        $params = self::validate_parameters(self::view_fastassignment_parameters(), $params);
     ...
     * @since Moodle 3.2
     */
    public static function view_fastassignment_returns() {
        return new external_single_structure(
            array(
                'status' => new external_value(PARAM_BOOL, 'status: true if success'),
                'warnings' => new external_warnings(),
