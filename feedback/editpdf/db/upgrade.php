 ...
 * @return bool
 */
function xmldb_fastassignfeedback_editpdf_upgrade($oldversion) {
    global $CFG, $DB;
    ...
    if ($oldversion < 2017022700) {
        ...
        $where = "component='fastassignfeedback_editpdf' AND filearea = 'importhtml' AND " . $sqllike;
        ...
        upgrade_plugin_savepoint(true, 2017022700, 'fastassignfeedback', 'editpdf');
    }
    ...
    if ($oldversion < 2018051401) {
        $table = new xmldb_table('fastassignfeedback_editpdf_queue');
        ...
        $sql = "SELECT MIN(id) as minid, submissionid, submissionattempt
                FROM {fastassignfeedback_editpdf_queue}
                GROUP BY submissionid, submissionattempt
                HAVING COUNT(id) > 1";

        if ($duplicatedrows = $DB->get_recordset_sql($sql)) {
            foreach ($duplicatedrows as $row) {
                $DB->delete_records_select('fastassignfeedback_editpdf_queue',
                    'submissionid = :submissionid AND submissionattempt = :submissionattempt AND id <> :minid', (array)$row);
            }
        }
        $duplicatedrows->close();

        // Define key submissionid-submissionattempt to be added to fastassignfeedback_editpdf_queue.
        $table = new xmldb_table('fastassignfeedback_editpdf_queue');
        $key = new xmldb_key('submissionid-submissionattempt', XMLDB_KEY_UNIQUE, ['submissionid', 'submissionattempt']);

        $dbman->add_key($table, $key);

        upgrade_plugin_savepoint(true, 2018051401, 'fastassignfeedback', 'editpdf');
    }
    ...
    if ($oldversion < 2019010800) {
        // Define table fastassignfeedback_editpdf_rot to be created.
        $table = new xmldb_table('fastassignfeedback_editpdf_rot');

        // Adding fields to table fastassignfeedback_editpdf_rot.
        ...
        // Adding keys to table fastassignfeedback_editpdf_rot.
        $table->add_key('primary', XMLDB_KEY_PRIMARY, ['id']);
        $table->add_key('gradeid', XMLDB_KEY_FOREIGN, ['gradeid'], 'fastassignment_grades', ['id']);

        // Adding indexes to table fastassignfeedback_editpdf_rot.
        $table->add_index('gradeid_pageno', XMLDB_INDEX_UNIQUE, ['gradeid', 'pageno']);

        // Conditionally launch create table for fastassignfeedback_editpdf_rot.
        if (!$dbman->table_exists($table)) {
            $dbman->create_table($table);
        }

        // Editpdf savepoint reached.
        upgrade_plugin_savepoint(true, 2019010800, 'fastassignfeedback', 'editpdf');
    }
