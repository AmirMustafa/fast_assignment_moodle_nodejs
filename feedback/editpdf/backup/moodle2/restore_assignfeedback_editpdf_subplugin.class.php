 ...
 * to restore one fastassignment_feedback subplugin.
 *
 * @package   fastassignfeedback_editpdf
 * @copyright 2013 Damyon Wiese
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class restore_fastassignfeedback_editpdf_subplugin extends restore_subplugin {
     ...
     * @param mixed $data
     */
    public function process_fastassignfeedback_editpdf_files($data) {
        $data = (object)$data;

        // In this case the id is the old gradeid which will be mapped.
        $this->add_related_files('fastassignfeedback_editpdf',
            \fastassignfeedback_editpdf\document_services::FINAL_PDF_FILEAREA, 'grade', null, $data->gradeid);
        $this->add_related_files('fastassignfeedback_editpdf',
            \fastassignfeedback_editpdf\document_services::PAGE_IMAGE_READONLY_FILEAREA, 'grade', null, $data->gradeid);
        $this->add_related_files('fastassignfeedback_editpdf', 'stamps', 'grade', null, $data->gradeid);
    }
     ...
     * @param mixed $data
     */
    public function process_fastassignfeedback_editpdf_annotation($data) {
        global $DB;
        ...
        $DB->insert_record('fastassignfeedback_editpdf_annot', $data);
     ...
     * @param mixed $data
     */
    public function process_fastassignfeedback_editpdf_comment($data) {
        global $DB;
        ...
        $DB->insert_record('fastassignfeedback_editpdf_cmnt', $data);
     ...
     * @param mixed $data
     */
    public function process_fastassignfeedback_editpdf_pagerotation($data) {
        global $DB;
        $data = (object)$data;
        $oldgradeid = $data->gradeid;
        $data->gradeid = $this->get_mappingid('grade', $oldgradeid);
        $DB->insert_record('fastassignfeedback_editpdf_rot', $data);
    }
}
