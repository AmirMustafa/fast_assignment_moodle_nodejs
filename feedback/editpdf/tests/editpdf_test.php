...
/**
 * Unit tests for fastassignfeedback_editpdf\comments_quick_list
 *
...
use \fastassignfeedback_editpdf\comments_quick_list;
use \fastassignfeedback_editpdf\document_services;
use \fastassignfeedback_editpdf\page_editor;
use \fastassignfeedback_editpdf\pdf;
use \fastassignfeedback_editpdf\comment;
use \fastassignfeedback_editpdf\annotation;
 ...
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignfeedback_editpdf_testcase extends advanced_testcase {
    ...
    protected function add_file_submission($student, $fastassignment) {
        global $CFG;
    ...
    public function test_page_editor() {
        ...
        $this->add_file_submission($student, $fastassignment);
    ...
    public function test_document_services() {
        ...
        document_services::get_page_images_for_attempt($fastassignment, $student->id, -1);
        $combinedpdf = $fs->get_file($contextid, $component, $filearea, $itemid, $filepath, $filename);
        ...
        $file = document_services::generate_feedback_document($fastassignment->get_instance()->id, $grade->userid, $grade->attemptnumber);
        $this->assertNotEmpty($file);

        $file2 = document_services::get_feedback_document($fastassignment->get_instance()->id, $grade->userid, $grade->attemptnumber);

        $this->assertEquals($file, $file2);

        document_services::delete_feedback_document($fastassignment->get_instance()->id, $grade->userid, $grade->attemptnumber);
        $file3 = document_services::get_feedback_document($fastassignment->get_instance()->id, $grade->userid, $grade->attemptnumber);
    ...
    public function test_conversion_task() {
        ...
        $task = new \fastassignfeedback_editpdf\task\convert_submissions;
        ...
        $fastassignment = $this->create_instance($course, $assignopts);
        ...
        $this->assertContains("Convert 1 submission attempt(s) for assignment {$fastassignment->get_instance()->id}", $output);
        $this->assertEquals(0, $DB->count_records('fastassignfeedback_editpdf_queue'));
        ...
        $this->assertEquals(1, $DB->count_records('fastassignfeedback_editpdf_queue'));

        // Fake some failed attempts for it.
        $queuerecord = $DB->get_record('fastassignfeedback_editpdf_queue', ['submissionid' => $submission->id]);
        $queuerecord->attemptedconversions = 3;
        $DB->update_record('fastassignfeedback_editpdf_queue', $queuerecord);
        ...
        $this->assertNotContains("Convert 1 submission attempt(s) for assignment {$fastassignment->get_instance()->id}", $output);
