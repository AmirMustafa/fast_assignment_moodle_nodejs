...
/**
 * Unit tests for fastassignfeedback_editpdf.
 *
...
use \fastassignfeedback_editpdf\page_editor;
use \mod_fastassignment\privacy\fastassignment_plugin_request_data;

/**
 * Unit tests for mod/fastassignment/feedback/editpdf/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignfeedback_editpdf_privacy_testcase extends \mod_fastassignment\tests\mod_fastassignment_privacy_testcase {

    public function setUp() {
        // Skip this test if ghostscript is not supported.
        $result = \fastassignfeedback_editpdf\pdf::test_gs_path(false);
        if ($result->status !== \fastassignfeedback_editpdf\pdf::GSPATH_OK) {
     ...
     */
    protected function create_feedback($fastassignment, $student, $teacher) {
        global $CFG;
        ...
        $pdfsubmission = (object) array(
            ...
            'component' => 'fastassignsubmission_file',
            'filearea' => FASTASSIGNSUBMISSION_FILE_FILEAREA,
            'itemid' => $submission->id,
            'filepath' => '/',
            'filename' => 'submission.pdf'
        );
        $sourcefile = $CFG->dirroot.'/mod/fastassignment/feedback/editpdf/tests/fixtures/submission.pdf';
        ...
        $plugin = $fastassignment->get_feedback_plugin_by_type('editpdf');

        $grade = $fastassignment->get_user_grade($student->id, true);

        $comment = new \fastassignfeedback_editpdf\comment();
        ...
        $annotation = new \fastassignfeedback_editpdf\annotation();
        ...
        $storedfile = \fastassignfeedback_editpdf\document_services::generate_feedback_document($fastassignment->get_instance()->id, $student->id,
                $grade->attemptnumber);
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('fastassignfeedback_editpdf');
        $collection = \fastassignfeedback_editpdf\privacy\provider::get_metadata($collection);
    ...
    public function test_export_feedback_user_data() {
        ...
        $fastassignment = $this->create_instance(['course' => $course,
                ...
                'fastassignfeedback_editpdf_enabled' => 1,
                'fastassignsubmission_file_maxsizebytes' => 1000000]);

        $context = $fastassignment->get_context();

        list($plugin, $grade, $storedfile) = $this->create_feedback($fastassignment, $user1, $user2);
        ...
        \fastassignfeedback_editpdf\privacy\provider::export_feedback_user_data($exportdata);
        // print_object($writer->get_files([get_string('privacy:path', 'fastassignfeedback_editpdf')]));
        // print_object($writer->get_files(['PDF feedback', $storedfile->get_filename()]));
        $pdffile = $writer->get_files([get_string('privacy:path', 'fastassignfeedback_editpdf')])[$storedfile->get_filename()];
    ...
    public function test_delete_feedback_for_context() {
        ...
        list($plugin1, $grade1, $storedfile1) = $this->create_feedback($fastassignment, $user1, $user3);
        list($plugin2, $grade2, $storedfile2) = $this->create_feedback($fastassignment, $user2, $user3);
        ...
        \fastassignfeedback_editpdf\privacy\provider::delete_feedback_for_context($requestdata);
    ...
    public function test_delete_feedback_for_grade() {
        ...
        \fastassignfeedback_editpdf\privacy\provider::delete_feedback_for_grade($requestdata);
    ...
    public function test_delete_feedback_for_grades() {
        ...
        $files = $DB->get_records('files', ['component' => 'fastassignfeedback_editpdf', 'filearea' => 'download']);
        ...
        \fastassignfeedback_editpdf\privacy\provider::delete_feedback_for_grades($deletedata);
