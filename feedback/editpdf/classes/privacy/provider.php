...
namespace fastassignfeedback_editpdf\privacy;
...
use \mod_fastassignment\privacy\fastassignfeedback_provider;
...
class provider implements
    ...
    public static function get_metadata(collection $collection) : collection {
        ...
        $collection->add_database_table('fastassignfeedback_editpdf_quick', $quickdata, 'privacy:metadata:tablepurpose');
    ...
    public static function get_context_for_userid_within_feedback(int $userid, contextlist $contextlist) {
        // This uses the fastassignment_grade table.
    }
    ...
    public static function export_feedback_user_data(fastassignment_plugin_request_data $exportdata) {
        ...
        $currentpath[] = get_string('privacy:path', 'fastassignfeedback_editpdf');
        $fastassignment = $exportdata->get_assign();
        $plugin = $fastassignment->get_plugin_by_type('fastassignfeedback', 'editpdf');
        $fileareas = $plugin->get_file_areas();
        $grade = $exportdata->get_pluginobject();
        foreach ($fileareas as $filearea => $notused) {
            writer::with_context($exportdata->get_context())
                    ->export_area_files($currentpath, 'fastassignfeedback_editpdf', $filearea, $grade->id);
        }
    }
    ...
    public static function delete_feedback_for_context(fastassignment_plugin_request_data $requestdata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files($requestdata->get_context()->id, 'fastassignfeedback_editpdf', $filearea);
        }
    ...
    public static function delete_feedback_for_grades(fastassignment_plugin_request_data $deletedata) {
        ...
        $fastassignment = $deletedata->get_assign();
        $plugin = $fastassignment->get_plugin_by_type('fastassignfeedback', 'editpdf');
        $fileareas = $plugin->get_file_areas();
        $fs = get_file_storage();
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete pdf files.
            $fs->delete_area_files_select($deletedata->get_context()->id, 'fastassignfeedback_editpdf', $filearea, $sql, $params);
        }

        // Remove table entries.
        $DB->delete_records_select('fastassignfeedback_editpdf_annot', "gradeid $sql", $params);
        $DB->delete_records_select('fastassignfeedback_editpdf_cmnt', "gradeid $sql", $params);
        $DB->delete_records_select('fastassignfeedback_editpdf_rot', "gradeid $sql", $params);
        // Submission records in fastassignfeedback_editpdf_queue will be cleaned up in a scheduled task
    }
}
