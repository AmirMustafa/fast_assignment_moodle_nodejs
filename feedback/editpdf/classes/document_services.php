...
/**
 * This file contains the ingest manager for the fastassignfeedback_editpdf plugin
 *
...
class document_services {
    ...
    /** Compoment name */
    const COMPONENT = "fastassignfeedback_editpdf";
...
EOD;
    ...
    private static function get_assignment_from_param($assignment) {
        ...
        if (!is_object($assignment)) {
            $cm = get_coursemodule_from_instance('fastassignment', $assignment, 0, false, MUST_EXIST);
            ...
            if ($plugin->is_enabled() && $plugin->is_visible()) {
                ...
                foreach ($pluginfiles as $filename => $file) {
                    if ($file instanceof \stored_file) {
                        ...
                        $record->component = 'fastassignfeedback_editpdf';
    ...
    public static function page_number_for_attempt($assignment, $userid, $attemptnumber, $readonly = false) {
        ...
        if ($readonly) {
            $grade = $assignment->get_user_grade($userid, true, $attemptnumber);
            $fs = get_file_storage();
            $files = $fs->get_directory_files($assignment->get_context()->id, 'fastassignfeedback_editpdf',
    ...
    protected static function generate_page_images_for_attempt($assignment, $userid, $attemptnumber, $resetrotation = true) {
        ...
        $tmpdir = \make_temp_directory('fastassignfeedback_editpdf/pageimages/' . self::hash($assignment, $userid, $attemptnumber));
    ...
    public static function generate_feedback_document($assignment, $userid, $attemptnumber) {
        ...
        $tmpdir = make_temp_directory('fastassignfeedback_editpdf/final/' . self::hash($assignment, $userid, $attemptnumber));
        $combined = $tmpdir . '/' . self::COMBINED_PDF_FILENAME;
        ...
        $fs = get_file_storage();
        $stamptmpdir = make_temp_directory('fastassignfeedback_editpdf/stamps/' . self::hash($assignment, $userid, $attemptnumber));
        $grade = $assignment->get_user_grade($userid, true, $attemptnumber);
     ...
     *
     * @param int|\fastassignment $assignment The assignment.
     ...
     * @param int|\fastassignment $assignment Assignment
    ...
    private static function save_jpg_to_pdf($assignment, $userid, $attemptnumber, $file, $size=null) {
        ...
        $tmpdir = make_temp_directory('fastassignfeedback_editpdf' . DIRECTORY_SEPARATOR
