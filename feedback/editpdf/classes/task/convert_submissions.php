 ...
 */
namespace fastassignfeedback_editpdf\task;
...
class convert_submissions extends scheduled_task {
    ...
    public function get_name() {
        return get_string('preparesubmissionsforannotation', 'fastassignfeedback_editpdf');
    }
    ...
    public function execute() {
        ...
        $records = $DB->get_records('fastassignfeedback_editpdf_queue');
        ...
        foreach ($records as $record) {
            ...
            $submission = $DB->get_record('fastassignment_submission', array('id' => $submissionid), '*', IGNORE_MISSING);
            if (!$submission || $record->attemptedconversions >= $conversionattemptlimit) {
                ...
                $DB->delete_records('fastassignfeedback_editpdf_queue', array('id' => $record->id));
                continue;
            }
            ...
            $DB->set_field('fastassignfeedback_editpdf_queue', 'attemptedconversions',
            ...
            if (empty($assignmentcache[$assignmentid])) {
                $cm = get_coursemodule_from_instance('fastassignment', $assignmentid, 0, false, MUST_EXIST);
