 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_feedback_editpdf extends fastassignment_feedback_plugin {
    ...
    public function get_name() {
        return get_string('pluginname', 'fastassignfeedback_editpdf');
    }
     ...
     * @param bool $readonly
     * @return fastassignfeedback_editpdf_widget
     */
    public function get_widget($userid, $grade, $readonly) {
        ...
        $widget = new fastassignfeedback_editpdf_widget($this->assignment->get_instance()->id,
                                                    $userid,
                                                    $attempt,
                                                    $url,
                                                    $filename,
    ...
    public function get_form_elements_for_user($grade, MoodleQuickForm $mform, stdClass $data, $userid) {
        ...
        $renderer = $PAGE->get_renderer('fastassignfeedback_editpdf');

        // Links to download the generated pdf...
        if ($attempt > -1 && page_editor::has_annotations_or_comments($grade->id, false)) {
            $html = $this->assignment->render_area_files('fastassignfeedback_editpdf',
                                                         document_services::FINAL_PDF_FILEAREA,
                                                         $grade->id);
            $mform->addElement('static', 'editpdf_files', get_string('downloadfeedback', 'fastassignfeedback_editpdf'), $html);
        }

        $widget = $this->get_widget($userid, $grade, false);

        $html = $renderer->render($widget);
        $mform->addElement('static', 'editpdf', get_string('editpdf', 'fastassignfeedback_editpdf'), $html);
        $mform->addHelpButton('editpdf', 'editpdf', 'fastassignfeedback_editpdf');
    ...
    public function is_empty(stdClass $grade) {
        ...
        $comments = $DB->count_records('fastassignfeedback_editpdf_cmnt', array('gradeid'=>$grade->id, 'draft'=>0));
        $annotations = $DB->count_records('fastassignfeedback_editpdf_annot', array('gradeid'=>$grade->id, 'draft'=>0));
    ...
    public function delete_instance() {
        global $DB;
        $grades = $DB->get_records('fastassignment_grades', array('assignment'=>$this->assignment->get_instance()->id), '', 'id');
        if ($grades) {
            list($gradeids, $params) = $DB->get_in_or_equal(array_keys($grades), SQL_PARAMS_NAMED);
            $DB->delete_records_select('fastassignfeedback_editpdf_annot', 'gradeid ' . $gradeids, $params);
            $DB->delete_records_select('fastassignfeedback_editpdf_cmnt', 'gradeid ' . $gradeids, $params);
        }
        return true;
    }
    ...
    public function is_available() {
        if ($this->enabledcache === null) {
            $testpath = fastassignfeedback_editpdf\pdf::test_gs_path(false);
            $this->enabledcache = ($testpath->status == fastassignfeedback_editpdf\pdf::GSPATH_OK);
        }
