...
namespace fastassignfeedback_file\privacy;
...
class provider implements
    ...
    public static function export_feedback_user_data(fastassignment_plugin_request_data $exportdata) {
        ...
        $currentpath[] = get_string('privacy:path', 'fastassignfeedback_file');
        $fastassignment = $exportdata->get_assign();
        $plugin = $fastassignment->get_plugin_by_type('fastassignfeedback', 'file');
        ...
        if ($filefeedback) {
            ...
            foreach ($fileareas as $filearea => $notused) {
                \core_privacy\local\request\writer::with_context($exportdata->get_context())
                        ->export_area_files($currentpath, 'fastassignfeedback_file', $filearea, $gradeid);
            }
        }
    }
    ...
    public static function delete_feedback_for_context(fastassignment_plugin_request_data $requestdata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            ...
            $fs->delete_area_files($requestdata->get_context()->id, 'fastassignfeedback_file', $filearea);
        }
    ...
    public static function delete_feedback_for_grades(fastassignment_plugin_request_data $deletedata) {
        ...
        foreach ($fileareas as $filearea => $notused) {
            // Delete feedback files.
            $fs->delete_area_files_select($deletedata->get_context()->id, 'fastassignfeedback_file', $filearea, $sql, $params);
        }

        // Delete table entries.
        $DB->delete_records_select('fastassignfeedback_file', "assignment = :assignment AND grade $sql", $params);
    }
}
