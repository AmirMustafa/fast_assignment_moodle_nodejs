 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignfeedback_file_zip_importer {

    /**
     * Is this filename valid (contains a unique participant ID) for import?
     *
     * @param fastassignment $assignment - The assignment instance
     ...
     * @param fastassignment_plugin $plugin - Set to the plugin that exported the file
     ...
     * @param fastassignment_plugin $plugin The matching plugin from the filename
    ...
    public function is_file_modified($assignment, $user, $plugin, $filename, $fileinfo) {
        ...
        } else if ($plugin->get_subtype() == 'fastassignfeedback') {
    ...
    public function extract_files_from_zip($zipfile, $contextid) {
        ...
        core_php_time_limit::raise(FASTASSIGNFEEDBACK_FILE_MAXFILEUNZIPTIME);
     ...
     *
     * @param fastassignment $assignment - The assignment instance
     * @param fastassignment_feedback_file $fileplugin - The file feedback plugin
    ...
    public function import_zip_files($assignment, $fileplugin) {
            ...
            if ($this->is_valid_filename_for_import($assignment, $unzippedfile, $participants, $user, $plugin, $filename)) {
                if ($this->is_file_modified($assignment, $user, $plugin, $filename, $unzippedfile)) {
                    ...
                    if ($oldfile = $fs->get_file($contextid,
                        ...
                        $newfilerecord->component = 'fastassignfeedback_file';
                        $newfilerecord->filearea = FASTASSIGNFEEDBACK_FILE_FILEAREA;
        ...
        require_once($CFG->dirroot . '/mod/fastassignment/feedback/file/renderable.php');
        $importsummary = new fastassignfeedback_file_import_summary($assignment->get_course_module()->id,
                                                            count($userswithnewfeedback),
                                                            $feedbackfilesadded,
                                                            $feedbackfilesupdated);

        $assignrenderer = $assignment->get_renderer();
        $renderer = $PAGE->get_renderer('fastassignfeedback_file');

        $o = '';

        $o .= $assignrenderer->render(new fastassignment_header($assignment->get_instance(),
                                                        $assignment->get_context(),
                                                        false,
                                                        $assignment->get_course_module()->id,
                                                        get_string('uploadzipsummary', 'fastassignfeedback_file')));
