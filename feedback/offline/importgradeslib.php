 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignfeedback_offline_grade_importer {
     ...
     * @param fastassignment $assignment The current assignment
     */
    public function __construct($importid, fastassignment $assignment, $encoding = 'utf-8', $separator = 'comma') {
    ...
    public function parsecsv($csvdata) {
        $this->csvreader = new csv_import_reader($this->importid, 'fastassignfeedback_offline');
    ...
    public function init() {
        ...
        $strid = get_string('recordid', 'fastassignment');
        $strmodified = get_string('lastmodifiedgrade', 'fastassignment');
    ...
    public function next() {
        ...
        while ($record = $this->csvreader->next()) {
            ...
            $id = substr($idstr, strlen(get_string('hiddenuser', 'fastassignment')));
