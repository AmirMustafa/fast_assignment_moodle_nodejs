...
define('FASTASSIGNSUBMISSION_ONLINETEXT_FILEAREA', 'submissions_onlinetext');
 ...
 * @license   http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignment_submission_onlinetext extends fastassignment_submission_plugin {
    ...
    public function get_name() {
        return get_string('onlinetext', 'fastassignsubmission_onlinetext');
    }
    ...
    private function get_onlinetext_submission($submissionid) {
        ...
        return $DB->get_record('fastassignsubmission_onlinetext', array('submission'=>$submissionid));
    }
    ...
    public function remove(stdClass $submission) {
        ...
        if ($submissionid) {
            $DB->delete_records('fastassignsubmission_onlinetext', array('submission' => $submissionid));
        }
        return true;
    }
    ...
    public function get_settings(MoodleQuickForm $mform) {
        ...
        $defaultwordlimit = 1000;       // set word limit
        $defaultwordlimitenabled = 1;
        ...
        $name = get_string('wordlimit', 'fastassignsubmission_onlinetext');

        // Create a text box that can be enabled/disabled for onlinetext word limit.
        $wordlimitgrp = array();
        $wordlimitgrp[] = $mform->createElement('text', 'fastassignsubmission_onlinetext_wordlimit', '', $options);
        $wordlimitgrp[] = $mform->createElement('checkbox', 'fastassignsubmission_onlinetext_wordlimit_enabled',
                '', get_string('enable'));
        $mform->addGroup($wordlimitgrp, 'fastassignsubmission_onlinetext_wordlimit_group', $name, ' ', false);
        $mform->addHelpButton('fastassignsubmission_onlinetext_wordlimit_group',
                              'wordlimit',
                              'fastassignsubmission_onlinetext');
        $mform->disabledIf('fastassignsubmission_onlinetext_wordlimit',
                           'fastassignsubmission_onlinetext_wordlimit_enabled',
                           'notchecked');
        $mform->hideIf('fastassignsubmission_onlinetext_wordlimit',
                       'fastassignsubmission_onlinetext_enabled',
                       'notchecked');

        // Add numeric rule to text field.
        $wordlimitgrprules = array();
        $wordlimitgrprules['fastassignsubmission_onlinetext_wordlimit'][] = array(null, 'numeric', null, 'client');
        $mform->addGroupRule('fastassignsubmission_onlinetext_wordlimit_group', $wordlimitgrprules);

        // Rest of group setup.
        $mform->setDefault('fastassignsubmission_onlinetext_wordlimit', $defaultwordlimit);
        $mform->setDefault('fastassignsubmission_onlinetext_wordlimit_enabled', $defaultwordlimitenabled);
        $mform->setType('fastassignsubmission_onlinetext_wordlimit', PARAM_INT);
        $mform->hideIf('fastassignsubmission_onlinetext_wordlimit_group',
                       'fastassignsubmission_onlinetext_enabled',
                       'notchecked');
    }
    ...
    public function save_settings(stdClass $data) {
        if (empty($data->fastassignsubmission_onlinetext_wordlimit) || empty($data->fastassignsubmission_onlinetext_wordlimit_enabled)) {
            $wordlimit = 0;
            $wordlimitenabled = 0;
        } else {
            $wordlimit = $data->fastassignsubmission_onlinetext_wordlimit;
            $wordlimitenabled = 1;
        }
    ...
    public function get_form_elements($submission, MoodleQuickForm $mform, stdClass $data) {
        ...
        $data = file_prepare_standard_editor($data,
                                             ...
                                             'fastassignsubmission_onlinetext',
                                             FASTASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
                                             $submissionid);
    ...
    public function save(stdClass $submission, stdClass $data) {
        ...
        $event = \fastassignsubmission_onlinetext\event\assessable_uploaded::create($params);
        $event->trigger();
        ...
        if ($onlinetextsubmission) {
            ...
            $updatestatus = $DB->update_record('fastassignsubmission_onlinetext', $onlinetextsubmission);
            $event = \fastassignsubmission_onlinetext\event\submission_updated::create($params);
            ...
            $onlinetextsubmission->id = $DB->insert_record('fastassignsubmission_onlinetext', $onlinetextsubmission);
            $params['objectid'] = $onlinetextsubmission->id;
            $event = \fastassignsubmission_onlinetext\event\submission_created::create($params);
            $event->set_assign($this->assignment);
            $event->trigger();
    ...
    public function get_editor_fields() {
        return array('onlinetext' => get_string('pluginname', 'fastassignsubmission_onlinetext'));
    }
    ...
    public function view_summary(stdClass $submission, & $showviewlink) {
        ...
        if ($onlinetextsubmission) {
            ...
            $text = $this->assignment->render_editor_content(FASTASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
                                                             $onlinetextsubmission->submission,
                                                             $this->get_type(),
                                                             'onlinetext',
                                                             'fastassignsubmission_onlinetext', true);
            ...
            if ($onlinetext != $shorttext) {
                $wordcount = get_string('numwords', 'fastassignsubmission_onlinetext', count_words($onlinetext));
    ...
    public function get_files(stdClass $submission, stdClass $user) {
        ...
        if ($onlinetextsubmission) {
            ...
            $filename = get_string('onlinetextfilename', 'fastassignsubmission_onlinetext');
    ...
    public function view(stdClass $submission) {
        ...
        if ($onlinetextsubmission) {
            ...
            $result .= $this->assignment->render_editor_content(FASTASSIGNSUBMISSION_ONLINETEXT_FILEAREA,
                                                                $onlinetextsubmission->submission,
                                                                $this->get_type(),
                                                                'onlinetext',
                                                                'fastassignsubmission_onlinetext');
    ...
    public function upgrade(context $oldcontext,
        ...
        if (!$DB->insert_record('fastassignsubmission_onlinetext', $onlinetextsubmission) > 0) {
    ...
    public function delete_instance() {
        global $DB;
        $DB->delete_records('fastassignsubmission_onlinetext',
    ...
    public function get_file_areas() {
        return array(FASTASSIGNSUBMISSION_ONLINETEXT_FILEAREA=>$this->get_name());
    }
    ...
    public function copy_submission(stdClass $sourcesubmission, stdClass $destsubmission) {
        ...
        $fs = get_file_storage();
        $files = $fs->get_area_files($contextid, 'fastassignsubmission_onlinetext',
                                     FASTASSIGNSUBMISSION_ONLINETEXT_FILEAREA, $sourcesubmission->id, 'id', false);
        foreach ($files as $file) {
            $fieldupdates = array('itemid' => $destsubmission->id);
            $fs->create_file_from_storedfile($fieldupdates, $file);
        }

        // Copy the fastassignsubmission_onlinetext record.
    ...
    public function check_word_count($submissiontext) {
        ...
        if ($wordcount <= $wordlimit) {
            ...
            $errormsg = get_string('wordlimitexceeded', 'fastassignsubmission_onlinetext',
