...
/**
 * Unit tests for fastassignsubmission_onlinetext.
 *
...
/**
 * Unit tests for mod/fastassignment/submission/onlinetext/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignsubmission_online_privacy_testcase extends \mod_fastassignment\tests\mod_fastassignment_privacy_testcase {
     ...
     */
    protected function create_online_submission($fastassignment, $student, $text) {
        global $CFG;
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('fastassignsubmission_onlinetext');
        $collection = \fastassignsubmission_onlinetext\privacy\provider::get_metadata($collection);
    ...
    public function test_export_submission_user_data() {
        ...
        list($plugin, $submission) = $this->create_online_submission($fastassignment, $user1, $submissiontext);
        ...
        \fastassignsubmission_onlinetext\privacy\provider::export_submission_user_data($exportdata);
        $this->assertEquals($submissiontext, $writer->get_data(['Attempt 1',
                get_string('privacy:path', 'fastassignsubmission_onlinetext')])->text);
    }
    ...
    public function test_delete_submission_for_context() {
        ...
        list($plugin, $submission) = $this->create_online_submission($fastassignment, $user1, $studenttext);
        $studenttext2 = 'Student two\'s text.';
        list($plugin2, $submission2) = $this->create_online_submission($fastassignment, $user2, $studenttext2);
        ...
        \fastassignsubmission_onlinetext\privacy\provider::delete_submission_for_context($requestdata);
    ...
    public function test_delete_submission_for_userid() {
        ...
        \fastassignsubmission_onlinetext\privacy\provider::delete_submission_for_userid($requestdata);
    ...
    public function test_delete_submissions() {
        ...
        $data = $DB->get_records('fastassignsubmission_onlinetext', ['assignment' => $assign1->get_instance()->id]);
        $this->assertCount(3, $data);
        // Delete the submissions for user 1 and 3.
        $requestdata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($context1, $assign1);
        $requestdata->set_userids([$user1->id, $user2->id]);
        $requestdata->populate_submissions_and_grades();
        \fastassignsubmission_onlinetext\privacy\provider::delete_submissions($requestdata);
        ...
        $data = $DB->get_records('fastassignsubmission_onlinetext', ['assignment' => $assign2->get_instance()->id]);
