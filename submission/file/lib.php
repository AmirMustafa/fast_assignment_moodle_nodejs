 ...
 */
function fastassignsubmission_file_pluginfile($course,
                                          $cm,
                                          context $context,
                                          $filearea,
                                          $args,
    ...
    $record = $DB->get_record('fastassignment_submission',
                              array('id'=>$itemid),
    ...
    if ($fastassignment->get_instance()->teamsubmission &&
        !$fastassignment->can_view_group_submission($groupid)) {
        return false;
    }

    if (!$fastassignment->get_instance()->teamsubmission &&
        !$fastassignment->can_view_submission($userid)) {
        return false;
    }

    $relativepath = implode('/', $args);

    $fullpath = "/{$context->id}/fastassignsubmission_file/$filearea/$itemid/$relativepath";
