...
/**
 * Unit tests for fastassignsubmission_file.
 *
...
/**
 * Unit tests for mod/fastassignment/submission/file/classes/privacy/
 *
 * @copyright  2018 Adrian Greeve <adrian@moodle.com>
 * @license    http://www.gnu.org/copyleft/gpl.html GNU GPL v3 or later
 */
class fastassignsubmission_file_privacy_testcase extends \mod_fastassignment\tests\mod_fastassignment_privacy_testcase {
     ...
     */
    protected function create_file_submission($fastassignment, $student, $filename) {
        global $CFG;
    ...
    public function test_get_metadata() {
        $collection = new \core_privacy\local\metadata\collection('fastassignsubmission_file');
        $collection = \fastassignsubmission_file\privacy\provider::get_metadata($collection);
    ...
    public function test_export_submission_user_data() {
        ...
        list($plugin, $submission) = $this->create_file_submission($fastassignment, $user1, $studentfilename);

        $writer = \core_privacy\local\request\writer::with_context($context);
        $this->assertFalse($writer->has_any_data());

        // The student should have a file submission.
        $exportdata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($context, $fastassignment, $submission, ['Attempt 1']);
        \fastassignsubmission_file\privacy\provider::export_submission_user_data($exportdata);
        // print_object($writer);
    ...
    public function test_delete_submission_for_context() {
        ...
        list($plugin2, $submission2) = $this->create_file_submission($fastassignment, $user2, $studentfilename);

        // Only need the context and fastassignment object in this plugin for this operation.
        $requestdata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($context, $fastassignment);
        \fastassignsubmission_file\privacy\provider::delete_submission_for_context($requestdata);
    ...
    public function test_delete_submission_for_userid() {
        ...
        // Only need the context and fastassignment object in this plugin for this operation.
        $requestdata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($context, $fastassignment, $submission, [], $user1);
        \fastassignsubmission_file\privacy\provider::delete_submission_for_userid($requestdata);
        // This checks that there are no files in this submission.
        $this->assertTrue($plugin->is_empty($submission));
    ...
    public function test_delete_submissions() {
        ...
        $data = $DB->get_records('files', ['contextid' => $context1->id, 'component' => 'fastassignsubmission_file']);
        $this->assertCount(6, $data);

        $data = $DB->get_records('fastassignsubmission_file', ['assignment' => $assign1->get_instance()->id]);
        $this->assertCount(3, $data);

        // Records in the second assignment (not being touched).
        $data = $DB->get_records('fastassignsubmission_file', ['assignment' => $assign2->get_instance()->id]);
        $this->assertCount(2, $data);

        $deletedata = new \mod_fastassignment\privacy\fastassignment_plugin_request_data($context1, $assign1);
        $deletedata->set_userids($userids);
        $deletedata->populate_submissions_and_grades();
        \fastassignsubmission_file\privacy\provider::delete_submissions($deletedata);
        $data = $DB->get_records('files', ['contextid' => $context1->id, 'component' => 'fastassignsubmission_file']);
        $this->assertCount(2, $data);
