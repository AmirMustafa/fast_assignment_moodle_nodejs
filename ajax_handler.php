...
if(isset($_GET["api_triggered"])){
    ...
    $check_prev_records = $DB->get_record_sql("SELECT * FROM {fastassignment_api_validation} WHERE activity = $activity AND user = $user AND role = $role");
    ...
    if(!empty($check_prev_records)) {
        ...
        $DB->update_record("fastassignment_api_validation",$record);
        ...
        $insertquery = $DB->insert_record('fastassignment_api_validation', $record);
    }
    ...
    $check_prev_records_again = $DB->get_record_sql("SELECT * FROM {fastassignment_api_validation} WHERE activity = $activity AND user = $user AND role = $role");
...
if(isset($_GET["studentView"])){
    ...
    $check_user_hits = $DB->get_record_sql("SELECT * FROM {fastassignment_api_validation} WHERE activity = $activity AND user = $user AND role = $role");
...
if(isset($_GET["teacherView"])){
    ...
    $get_submitteddesc_sql = $DB->get_record_sql(
        "SELECT onlinetext FROM {fastassignsubmission_onlinetext} WHERE assignment = $activity AND submission = $submission_id"
    );
    
    if(!empty($get_submitteddesc_sql)) {
        $submission_desc = $get_submitteddesc_sql->onlinetext;
        // $a = strip_tags($submission_desc);
        // $submission_desc = html_entity_decode($a);
    } else {
