...
$(document).ready(function () {
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      let remainingautoeval_hits =
        total_autoeval_teacher_hits - autoeval_used_hits;
      ...
      const automateEval = async (dataToCheck, testlink) => {
        ...
        $.ajax({
          ...
          success: function (response) {
            ...
            if (teacherPermission == 1 && isAdmin == 0) {
              $("#apimessage").html(
                "No. of automated evaluations available: " +
                  remainingautoeval_hits
              );
            }
          },
        });
          ...
          if (checkAvailableAutoevalAPI || isAdmin == 1) {
            ...
            automateEval(dataToCheck, testlink).then((response) => {
              ...
              if (status) {
                ...
                // Writing Templates
                let templateStatistics = getTemplate(
                  result,
                  testlink,
                  "statistics"
                );
                ...
                let templateParagraph = getTemplate(
                  result,
                  testlink,
                  "paragraph"
                );
                ...
                let templateScolarship = getTemplate(
                  result,
                  testlink,
                  "scholarship"
                );
